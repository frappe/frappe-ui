...
/**
 * zrender measures label text on a detached canvas and keeps every result in a
 * module-level LRU keyed by the font string. A width measured before the
 * webfont arrives is therefore measured against the fallback font and never
 * measured again — no `resize()` and no fresh `setOption` can correct it, which
 * is how the widest axis labels end up clipped. The one measurement a chart
 * gets has to happen after the fonts settle, so that is when it is built. The
 * timeout is the guard: a font request that never resolves must not keep the
 * chart off the page.
 */
