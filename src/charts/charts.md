...
[echarts](https://echarts.apache.org). Props are flat and name columns of your
data — `x`, `y`, `series`, `category`, `value` — rather than taking a config
object. A saved or serialized chart is one typed object spread with
`v-bind="savedChart"`.
...
BarChart never pays for the rest of the library. Nothing imports `echarts`
wholesale.
