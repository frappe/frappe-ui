...
export function useCall(options) {
    const { url, method = 'GET', params, immediate = true, refetch = false, baseUrl = '', initialData, cacheKey, staleOnError = false, transform, beforeSubmit, onSuccess, onError, } = options;
    ...
    let result;
    if (method === 'POST') {
        result = useFrappeFetch(computedUrl, fetchOptions).post(computedParams);
    }
    else if (method === 'PUT') {
        result = useFrappeFetch(computedUrl, fetchOptions).put(computedParams);
    }
    else if (method === 'DELETE') {
        result = useFrappeFetch(computedUrl, fetchOptions).delete(computedParams);
    }
    else {
        result = useFrappeFetch(computedUrl, fetchOptions).get();
    }
    const { data, error, isFetching, isFinished, canAbort, aborted, abort, execute: _execute, onFetchResponse, onFetchError, } = result;
    function execute() {
