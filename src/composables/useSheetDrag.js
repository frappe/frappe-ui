 ...
 */
const NON_DRAGGABLE = 'input, textarea, select, [contenteditable=""], [contenteditable="true"], [data-no-sheet-drag]';
/**
 ...
 */
function rubberband(overscroll, dimension, constant = 0.55) {
 ...
 */
function findScroller(from, root) {
    let el = from;
    while (el) {
...
export function useSheetDrag(options) {
    ...
    function begin(x, y, from) {
        const el = toValue(target);
        if (!el || closing || !toValue(enabled))
            return false;
        const origin = from instanceof Element ? from : null;
        if (origin?.closest(NON_DRAGGABLE))
            return false;
     ...
     */
    function move(x, y) {
        if (phase !== 'undecided' && phase !== 'dragging')
            return false;
        const dx = x - startX;
        const dy = y - startY;
        if (phase === 'undecided') {
            ...
            if (!fromHandle) {
                ...
                if (dy <= 0)
                    return reject();
                if (scroller && scrollTopAtStart > 0)
                    return reject();
            }
            phase = 'dragging';
            dragging.value = true;
    ...
    function onTouchStart(e) {
        if (phase !== 'idle' || e.touches.length !== 1)
            return;
    ...
    function onPointerDown(e) {
        ...
        if (phase !== 'idle' && !captured && !touchGesture)
            phase = 'idle';
        if (phase !== 'idle')
            return;
        if (!begin(e.clientX, e.clientY, e.target))
            return;
