 ...
 * ```
 */
export const shellScrollContainer = computed(() => containers.value[containers.value.length - 1] ?? null);
/** Register a shell's scroll element. Internal to the two shell components. */
export function registerShellScrollContainer(el) {
    containers.value = [...containers.value, el];
}
/** Remove a previously registered scroll element. Internal to the two shells. */
export function unregisterShellScrollContainer(el) {
...
export function useShellScrolled(options = {}) {
    ...
    watch(shellScrollContainer, (el, prev) => {
    ...
    let warnTimer = null;
    onMounted(() => {
        if (import.meta.env.PROD || warnedNoShell)
            return;
        warnTimer = setTimeout(() => {
            warnTimer = null;
            if (shellScrollContainer.value || warnedNoShell)
                return;
            warnedNoShell = true;
            console.warn('[frappe-ui] useShellScrolled() found no app shell, so it will stay false. ' +
