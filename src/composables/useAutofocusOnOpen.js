import { watch, nextTick, toValue } from 'vue';
/**
 ...
 */
export function useAutofocusOnOpen(isOpen, getContainer) {
    ...
    watch(() => toValue(isOpen), async (open) => {
        ...
        requestAnimationFrame(() => {
            ...
            if (!root)
                return;
            const marker = root.querySelector('[autofocus]');
            if (!marker)
                return;
            const target = marker.matches(FOCUSABLE_SELECTOR)
                ? marker
                : marker.querySelector(FOCUSABLE_SELECTOR);
            if (!target)
                return;
            target.focus();
