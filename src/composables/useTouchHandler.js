...
export function useTouchHandler(options) {
    const { targetRef, zoomLevel = ref(100), onSwipeLeft, onSwipeRight, onDoubleTap, onTap, onPanStart, onPanMove, onPanAnimate, onPanEnd, onPinchStart, onPinchMove, onPinchEnd, doubleTapDelay = 300, minSwipeDistance = 50, maxVerticalSwipeDistance = 75, maxTapDuration = 200, maxTapMovement = 10, panThreshold = 5, // Pixels moved before pan starts
    ...
    const handleTouchStart = (event) => {
        ...
        if (event.touches.length === 1) {
            ...
            touchStartDistance.value = Math.hypot(touch2.clientX - touch1.clientX, touch2.clientY - touch1.clientY);
    ...
    const handleTouchMove = (event) => {
        if (!initialTouchPoints.value)
            return; // Should have initial points if touch started correctly
    ...
    const handleTouchEnd = (event) => {
        ...
        if (wasPanning &&
            ...
            if (velocityMagnitude > inertiaVelocityThreshold &&
                onPanAnimate &&
                ...
                const animate = (currentTime) => {
                    ...
                    const dampingFactor = Math.pow(Math.min(0.999, inertiaDamping), // Ensure base is < 1
                    ...
                    if (Math.hypot(animVelocity.x, animVelocity.y) < 0.01 ||
                        zoomLevel.value <= 100) {
