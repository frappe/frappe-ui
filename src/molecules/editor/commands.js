...
export const hasMark = (name) => (editor) => name in editor.schema.marks;
export const hasNode = (name) => (editor) => name in editor.schema.nodes;
export const hasExtension = (name) => (editor) => editor.extensionManager?.extensions?.some((extension) => extension.name === name) ?? true;
export const hasCommand = (name) => (editor) => typeof editor.commands[name] === 'function';
const headingIcon = {
    1: 'lucide-heading-1',
    2: 'lucide-heading-2',
    3: 'lucide-heading-3',
...
export function headingMeta(level) {
    return {
        label: `Heading ${level}`,
...
export const commandMeta = {
    ...
    image: {
        label: 'Image / Gallery',
        icon: 'lucide-image',
        isAvailable: (editor) => hasNode('image')(editor) && hasCommand('selectAndUploadImage')(editor),
    },
    video: {
        label: 'Video',
        icon: 'lucide-video',
        isAvailable: (editor) => hasNode('video')(editor) && hasCommand('selectAndUploadVideo')(editor),
    },
    attachment: {
        label: 'Attachment',
        icon: 'lucide-paperclip',
        isAvailable: (editor) => hasNode('attachment')(editor) &&
    ...
    link: {
        ...
        isAvailable: (editor) => hasMark('link')(editor) && hasCommand('openLinkEditor')(editor),
    },
    table: {
        label: 'Table',
        icon: 'lucide-table',
    ...
    embed: {
        ...
        isAvailable: (editor) => hasNode('iframe')(editor) && hasCommand('openIframeDialog')(editor),
    },
    toc: {
