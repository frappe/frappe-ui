...
export function platformByName(name) {
    ...
    return (PLATFORM_CONFIGS.find((config) => config.name.toLowerCase() === lower) ??
        null);
}
...
export function detectPlatform(url) {
    ...
    return (PLATFORM_CONFIGS.find((config) => matchesHostname(host, config.hosts)) ??
        null);
}
/** Compute the natural aspect ratio + default size for a URL's platform. */
export function calculateAspectRatio(url) {
...
export function getOptimalDimensions(url, containerWidth) {
...
export function processEmbedUrl(url) {
    const platform = detectPlatform(url);
...
function convertYouTube(url) {
    ...
    if (start)
        params.set('start', start.replace(/s$/, ''));
...
function convertVimeo(url) {
    ...
    if (matchesHostname(parsed.hostname, ['player.vimeo.com']))
        return url;
    const segments = parsed.pathname.split('/').filter(Boolean);
