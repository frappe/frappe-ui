...
import { parseIframeFromHTML, } from './parseIframeEmbed';
/** Insert a validated iframe node at the current selection. */
function insertIframeNode(view, nodeName, embed, allowlist) {
...
export function createIframePastePlugin(nodeName, allowlist) {
    return new Plugin({
        ...
        props: {
            handlePaste: (view, event) => {
                ...
                if (html) {
                    ...
                    if (embed && insertIframeNode(view, nodeName, embed, allowlist))
                        return true;
                }
                ...
                if (text) {
                    ...
                    if (validateIframeUrl(processed, { allowlist })) {
                        if (insertIframeNode(view, nodeName, { src: trimmed }, allowlist))
                            return true;
                    }
                }
                return false;
