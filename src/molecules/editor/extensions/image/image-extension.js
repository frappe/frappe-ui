...
import { resolveUploadOptions as resolveUploadOptionsBase, } from '#molecules/editor/extensions/shared/media-upload-engine';
...
export const ImageExtension = NodeExtension.create({
    ...
    addCommands() {
        const resolve = (editor) => resolveUploadOptionsBase({ ...this.options, editor });
        return {
            setImageAlign: (align) => ({ commands }) => commands.updateAttributes(this.name, { align }),
            setImageFloat: (float) => ({ commands }) => commands.updateAttributes(this.name, { float }),
            setImage: (attributes) => ({ commands, editor }) => {
                ...
                if (result && attributes.src) {
                    ...
                    if (pos !== null)
                        backfillFromSrc(editor, pos, attributes.src);
                }
                return result;
            },
...
function backfillFromSrc(editor, pos, src) {
