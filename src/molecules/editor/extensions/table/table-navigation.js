...
import { CellSelection, TableMap, cellAround, columnResizingPluginKey, goToNextCell, isInTable, nextCell, selectionCell, } from '@tiptap/pm/tables';
 ...
 */
function arrowLeavesCell(view, event) {
...
function editCell(editor, $cell, at) {
    const { state, view } = editor;
    const cell = $cell.nodeAfter;
    if (!cell)
        return false;
 ...
 */
function exitTable(editor, dir) {
    const { state, view } = editor;
    const $cell = selectionCell(state);
    let depth = $cell.depth;
    while (depth > 0 && $cell.node(depth).type.name !== 'table')
        depth--;
    if (!depth)
        return true;
 ...
 */
function moveCell(editor, axis, dir) {
    const { state } = editor;
    const $next = nextCell(selectionCell(state), axis, dir);
    if ($next)
        return selectCell(editor, $next.pos);
    if (axis === 'vert')
        return exitTable(editor, dir);
    return true;
}
/**
 ...
 */
function extendCell(editor, axis, dir) {
    const { state, view } = editor;
...
function tabToCell(editor, dir) {
    ...
    if (!goToNextCell(dir)(state, view.dispatch)) {
        ...
        if (!goToNextCell(dir)(editor.state, view.dispatch))
            return true;
    }
    const $cell = cellAround(editor.state.selection.$head);
    if ($cell)
        selectCell(editor, $cell.pos);
    return true;
}
/** Select every cell of the table containing `$cell`. */
function selectWholeTable(editor, $cell) {
    const { state, view } = editor;
    const { start, map } = tableAround($cell);
    view.dispatch(state.tr.setSelection(new CellSelection(state.doc.resolve(start + map.map[0]), state.doc.resolve(start + map.map[map.map.length - 1]))));
    return true;
}
/**
                ...
                props: {
                    handleKeyDown(view, event) {
                        ...
                        switch (event.key) {
                            ...
                            case 'Enter':
                                if (navigating) {
                                    return editCell(editor, state.selection.$anchorCell);
                                }
                    ...
                    handleDOMEvents: {
                        ...
                        pointerdown(view, event) {
                            ...
                            const wrapper = anchorDom instanceof HTMLElement
                            ...
                            trackPointerDrag({
                                ...
                                onPoint(x, y) {
                                    ...
                                    if ($head.start(-1) !== $anchor.start(-1))
                                        return;
