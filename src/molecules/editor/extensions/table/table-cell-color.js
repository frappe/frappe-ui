 ...
 */
function cellContentRanges(state) {
    const ranges = [];
    const addCell = (cell, pos) => {
        if (!cell)
            return;
    ...
    if (selection instanceof CellSelection) {
        ...
        if ($cell)
            addCell($cell.nodeAfter, $cell.pos);
    }
    return ranges;
}
/** True when every text node in `[from, to)` already carries `markType`. */
function isRangeFullyMarked(state, from, to, markType) {
    let hasText = false;
    let allMarked = true;
...
export const cellBackgroundAttributes = {
    backgroundColor: {
        ...
        renderHTML: (attributes) => {
            ...
            if (typeof name !== 'string' || !PALETTE_NAMES.includes(name))
                return {};
        ...
        return {
            ...
            setCellTextColor: (colorName) => ({ state, tr, dispatch }) => {
                ...
                for (const { from, to } of ranges) {
                    ...
                    if (colorName)
                        tr.addMark(from, to, markType.create({ color: colorName }));
                }
                if (dispatch)
                    dispatch(tr.scrollIntoView());
                return true;
            },
            setCellTextAlign: (alignment) => ({ state, tr, dispatch }) => {
                if (!isInTable(state))
                    return false;
                const ranges = cellContentRanges(state);
                if (!ranges.length)
            ...
            toggleCellBold: () => ({ state, tr, dispatch }) => {
                if (!isInTable(state))
                    return false;
                ...
                const allBold = ranges.every(({ from, to }) => from >= to ? true : isRangeFullyMarked(state, from, to, markType));
                for (const { from, to } of ranges) {
                    if (from >= to)
                        continue;
                    if (allBold)
