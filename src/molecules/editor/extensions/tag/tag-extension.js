...
import { insertSuggestionNode, filterByQuery, getSuggestionOptions, } from '#molecules/editor/extensions/shared/suggestion-helpers';
...
export const TagExtension = createSuggestionExtension({
    ...
    items: ({ query, editor }) => {
        const options = getSuggestionOptions(editor, 'tagSuggestion');
        const tags = toValue(options?.tags ?? []);
        const filteredTags = filterByQuery(tags, query, 'label').map((tag) => ({ ...tag, display: tag.label }));
        if (query.length > 0 &&
 ...
 */
function buildTagExtensions(items) {
    if (items == null)
        return [TagNode];
