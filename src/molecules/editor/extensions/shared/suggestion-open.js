 ...
 */
function isStrandedPadding(doc, pos) {
    if (pos + 1 > doc.content.size)
        return false;
    const padding = doc.resolve(pos).nodeAfter;
    if (!padding?.isText || !padding.text?.startsWith(' '))
        return false;
 ...
 */
export function insertSuggestionTrigger(tr, char) {
...
export function autoOpenCleanupPlugin(options) {
    ...
    const key = new PluginKey('suggestionAutoOpenCleanup');
    ...
    const suggestionState = (state) => pluginKey.getState(state);
    return new Plugin({
        key,
        state: {
            init: () => null,
            apply(tr, marker) {
                const meta = tr.getMeta(AUTO_OPEN_META);
                if (meta === null)
                    return null;
                if (meta?.char === char) {
        ...
        appendTransaction(transactions, oldState, newState) {
            ...
            if (suggestionState(newState)?.active)
                return null;
