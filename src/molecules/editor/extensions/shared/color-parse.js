 ...
 */
export function parseRgbToRgb(rgb) {
 ...
 */
export function getClosestNamedColor(rgb, colorMap, allowed) {
    let closest = null;
    ...
    for (const name of candidates) {
        ...
        if (!anchor)
            continue;
        const distance = Math.sqrt((rgb.r - anchor.r) ** 2 +
            (rgb.g - anchor.g) ** 2 +
            (rgb.b - anchor.b) ** 2);
    ...
    if (closest === null || closestAnchor === null)
        return null;
    if (EXCLUDE_FROM_PASTE.includes(closest))
        return null;
    if (chroma(rgb) < chroma(closestAnchor) * NEUTRAL_CHROMA_RATIO)
        return null;
    return closest;
}
/**
 ...
 */
export function matchLegacyHex(hex, variant) {
    ...
    const name = map[key];
    if (EXCLUDE_FROM_PASTE.includes(name))
