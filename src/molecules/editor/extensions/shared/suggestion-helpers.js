 ...
 */
export function insertSuggestionNode(editor, range, nodeType, attrs, opts = {}) {
    const { trailingSpace = true } = opts;
    const content = [{ type: nodeType, attrs }];
    if (trailingSpace) {
 ...
 */
export function filterByQuery(items, query, key) {
 ...
 */
export function isInCode(doc, pos) {
 ...
 */
export function getSuggestionOptions(editor, extensionName) {
