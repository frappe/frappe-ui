...
function warnMonochromeOnce() {
    ...
    console.warn('[namedHighlight] multicolor is disabled; applying a plain highlight and ignoring the color name.');
}
/**
...
export const NamedHighlightExtension = Mark.create({
            ...
            color: {
                ...
                parseHTML: (element) => {
                    ...
                    if (style) {
                        ...
                        const fromStyle = extractHighlightColorFromStyle(style, this.options.colors);
                        if (fromStyle)
                            return fromStyle;
                    }
                    ...
                    if (legacyColorAttr) {
                        const fromLegacy = extractHighlightColorFromStyle(`background-color: ${legacyColorAttr}`, this.options.colors);
                        if (fromLegacy)
                            return fromLegacy;
                    }
                    return null;
    ...
    addCommands() {
        return {
            setHighlightByName: (colorName) => ({ chain, state }) => {
            ...
            toggleHighlightByName: (colorName) => ({ chain, editor, state }) => {
                if (!this.options.colors.includes(colorName)) {
                    console.warn(`Highlight color "${colorName}" is not in the allowed colors list`);
                    return false;
                }
