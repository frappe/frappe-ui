...
import { commandMeta, headingMeta, } from '#molecules/editor/commands';
...
function slashCommand(meta, command) {
    return {
        title: meta.label,
...
function inGroup(group, items) {
    return items.map((item) => ({ ...item, group }));
}
const getCommands = () => [
    ...inGroup('Text', [
        slashCommand(headingMeta(2), ({ editor, range }) => {
            editor
                .chain()
                .focus()
                .deleteRange(range)
        ...
        slashCommand(headingMeta(3), ({ editor, range }) => {
            editor
                .chain()
                .focus()
                .deleteRange(range)
    ...
    ...inGroup('Embeds', [
        ...
        ...PLATFORM_CONFIGS.map((platform) => slashCommand({
            label: platform.name,
            icon: platform.icon,
            isAvailable: (editor) => commandMeta.embed.isAvailable(editor) &&
    ...
    ...inGroup('Insert', [
        ...
        slashCommand({ ...commandMeta.table, icon: 'lucide-table-2' }, ({ editor, range }) => {
...
export const SlashCommands = createSuggestionExtension({
    ...
    items: ({ query, editor }) => {
        return filterByQuery(getCommands().filter((item) => item.isAvailable?.(editor) !== false), query, 'title');
    },
