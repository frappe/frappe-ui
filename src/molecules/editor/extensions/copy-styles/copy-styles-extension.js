...
import { applyMarksAndAttrs, clearMarksAndAttrs, collectBlockAttrs, collectMarks, } from './style-clipboard-utils';
const StyleClipboardExtension = Extension.create({
    ...
    addCommands() {
        return {
            ...
            applyStyles: () => ({ editor, tr, dispatch }) => {
                const { state } = editor;
                const { from, to } = state.selection;
                if (from === to)
                    return false;
            ...
            clearStyles: () => ({ editor, tr, dispatch }) => {
                const { state } = editor;
                const { from, to } = state.selection;
                if (from === to)
                    return false;
    ...
    addKeyboardShortcuts() {
        return {
            Escape: () => {
                if (!this.storage.data && !this.storage.painting)
                    return false;
                this.storage.data = null;
                this.storage.painting = false;
    ...
    addProseMirrorPlugins() {
        ...
        return [
            new Plugin({
                view(view) {
                    const handleMouseUp = () => {
                        if (!extension.storage.painting || !extension.storage.data)
                            return;
