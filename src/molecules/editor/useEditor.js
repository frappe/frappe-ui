import { onBeforeUnmount, shallowRef, toRaw, toValue, watch, } from 'vue';
...
export function useEditor(options) {
    ...
    const isCollaborationMode = options.extensions.some((extension) => extension.name === 'collaboration');
    ...
    if (import.meta.env?.DEV &&
        ...
        console.warn("[frappe-ui] format: 'markdown' needs the Markdown extension in `extensions` — import { Markdown } from 'frappe-ui/editor'.");
    }
    const serialize = (tiptapEditor) => format === 'json'
    ...
    const editorOptions = {
        ...
        onFocus: ({ editor: tiptapEditor, event }) => {
            options.onFocus?.(tiptapEditor, event);
        },
        onBlur: ({ editor: tiptapEditor, event }) => {
            options.onBlur?.(tiptapEditor, event);
        },
    ...
    editor.value = new TiptapEditor(editorOptions);
    const editorStorage = editor.value.storage;
    ...
    if (!isCollaborationMode && options.content) {
        watch(options.content, (content) => {
            ...
            if (toRaw(content) === lastEmitted)
                return;
            if (format === 'html' && editor.value.getHTML() === content)
                return;
            if (format === 'markdown' && editor.value.getMarkdown() === content)
                return;
            applyingExternalUpdate = true;
            try {
                editor.value.commands.setContent(content ?? '', format === 'markdown'
    ...
    watch(() => toValue(options.editable), (editable) => {
