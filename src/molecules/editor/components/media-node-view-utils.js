 ...
 */
export function wrapperClasses(float) {
    if (!float)
        return ['my-2'];
 ...
 */
export function containerClasses(attrs, selected) {
 ...
 */
export function aspectRatioFrom(width, height) {
    if (!width || !height)
        return undefined;
    return `${width} / ${height}`;
}
 ...
 */
export function heightOverWidth(width, height) {
    if (!width || !height)
        return 1;
    return height / width;
}
