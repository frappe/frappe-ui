...
import { Table as TiptapTable, TableRow, TableCell as TiptapTableCell, TableHeader as TiptapTableHeader, } from '@tiptap/extension-table';
import { columnResizing, tableEditing, TableView } from '@tiptap/pm/tables';
import { cellBackgroundAttributes, TableCellColor as TableCellColorExtension, } from './extensions/table/table-cell-color';
...
import { MentionExtension, } from './extensions/mention/mention-extension';
import { TagComposite } from './extensions/tag';
...
function pushConfigured(list, extension, option) {
    if (option !== false)
        list.push(extension.configure(option ?? {}));
}
/**
 ...
 */
export function setPlaceholder(editor, text) {
    const storage = placeholderStorage(editor);
    if (!editor || !storage || storage.text === text)
        return;
    storage.text = text;
