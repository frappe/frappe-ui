 ...
 */
export function useTocActiveHeading(editor, anchors, container) {
    // A monotonically-bumped tick that invalidates the computed on scroll.
    const scrollTick = ref(0);
    const enrichedAnchors = computed(() => {
        ...
        return anchors.value.map((anchor) => {
            ...
            const isActive = relativeTop >= scrollTop + ACTIVE_TOP_THRESHOLD &&
    ...
    const stopContainerWatch = watch(container, (el, _prev, onCleanup) => {
        if (!el)
            return;
