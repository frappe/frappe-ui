...
import { collectHeadings, getActiveTabId, getActiveTabRange, } from '#molecules/editor/extensions/shared/heading-scope';
 ...
 */
export function useTocAnchors(editor) {
    const anchors = ref([]);
    const anchorTree = computed(() => foldHeadings(anchors.value, (item) => item));
    const refresh = () => {
    ...
    const stopDocWatch = watch(() => editor.state?.doc, refresh);
    // Active-tab changes don't necessarily mutate the doc, so watch the id too.
    const stopTabWatch = watch(() => getActiveTabId(editor), refresh);
    onMounted(() => {
        refresh();
