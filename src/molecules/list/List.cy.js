...
import { List, ListCell, ListHeader, ListHeaderCell, ListHeaderCellSort, ListRow, ListRows, } from './index';
function makeRouter() {
    return createRouter({
...
describe('List (selection)', () => {
    function mountSelectable(rowProps = {}) {
        const selection = ref([]);
        ...
        cy.mount({
            render: () => h(List, {
                ...
                'onUpdate:selection': (next) => (selection.value = next),
...
describe('List (select all)', () => {
    ...
    function mountSelectAll(initial = []) {
        ...
        cy.mount({
            render: () => h(List, {
                ...
                h(ListRows, { items: items.value }, {
                    default: ({ item, value, }) => h(ListRow, { value }, () => h(ListCell, () => item.id)),
                }),
            ]),
        });
    ...
    it('shows the mixed state when only some rows are selected', () => {
        mountSelectAll(['2']);
        cy.get('[data-slot=list-header-checkbox]').should('have.attr', 'aria-checked', 'mixed');
        ...
        cy.get('[data-slot=list-header-checkbox] input[type=checkbox]')
            .first()
            .should(($el) => {
            expect($el[0].indeterminate).to.equal(true);
        });
    });
    ...
    it('honors a custom rowKey when the value is neither name nor id', () => {
        ...
        cy.mount({
            render: () => h(List, {
                ...
                h(ListHeader, () => h(ListHeaderCell, () => 'Name')),
                h(ListRows, { items, rowKey: 'ref' }, {
                    default: ({ item, value, }) => h(ListRow, { value }, () => h(ListCell, () => item.ref)),
                }),
            ]),
        });
        cy.get('[data-slot=list-header-checkbox]')
...
describe('List (active row)', () => {
    ...
    function mountActive(rowProps = {}) {
        const active = ref('2');
        cy.mount({
            render: () => h(List, {
                active: active.value,
                'onUpdate:active': (next) => (active.value = next),
            }, () => [
                feedRow('1', rowProps),
                feedRow('2', rowProps),
                feedRow('3', rowProps),
...
describe('List (column mode)', () => {
    function mountTable() {
        ...
        function toggleSort(field, firstDirection = 'asc') {
        ...
        const directionFor = (field) => sortField.value === field ? sortDirection.value : null;
        cy.mount({
            render: () => h(List, { columns: ['minmax(0,1fr)', '10rem', '4rem'] }, () => [
                h(ListHeader, () => [
                    h(ListHeaderCellSort, {
                        direction: directionFor('name'),
                        onClick: () => toggleSort('name'),
                    }, {
                        default: () => 'User',
                        // Adornments are app-supplied; expose the scoped direction for assertions.
                        suffix: ({ direction }) => h('span', { 'data-testid': 'sort-icon' }, direction ?? 'none'),
                    }),
                    h(ListHeaderCellSort, {
    ...
    it('shares the column template between header and rows', () => {
        ...
        cy.get('[data-slot=list-header]').should(($header) => {
            const rowTemplate = getComputedStyle($header[0].parentElement.querySelector('[data-slot=list-row]')).gridTemplateColumns;
    ...
    it('right-aligns an end-aligned sort cell and leads with the glyph', () => {
        ...
        cy.mount({
            render: () => h(List, { columns: ['minmax(0,1fr)', '6rem', '4rem'] }, () => [
                h(ListHeader, () => [
                    h(ListHeaderCell, () => 'Name'),
                    h(ListHeaderCellSort, {
                        direction: direction.value,
                        align: 'end',
                        onClick: () => (direction.value = direction.value ? null : 'asc'),
                    }, {
                        default: () => 'Size',
        ...
        sizeCell()
            ...
            expect(html.indexOf('data-testid="glyph"')).to.be.lessThan(html.indexOf('Size'));
        });
        ...
        cy.mount({
            render: () => h('div', {
                ...
                h(List, { rowHeight: 40, columns: ['minmax(0,1fr)'] }, () => [
                    h(ListRows, { items, virtual: true }, {
                        default: ({ item }) => h(ListRow, { key: item.id }, () => [
