 ...
 */
export function isLucideIconString(icon) {
 ...
 * `🚀`, `✨`, `⚠️`, `❌`, etc.
 */
export function isEmojiIconString(icon) {
...
export function isFeatherIconString(icon) {
    if (typeof icon !== 'string' || !icon)
        return false;
    if (isLucideIconString(icon))
        return false;
 ...
 */
export function warnFeatherIconUsage(component, prop, value) {
    if (!isFeatherIconString(value))
        return;
    warnDeprecated(`${component}.${prop} feather-name icon`, `a lucide-* string (e.g. "lucide-${value}") or a passed Component`);
}
