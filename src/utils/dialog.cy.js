...
const DialogManager = defineComponent({
    name: 'DialogManager',
    setup() {
        return () => h('div', dialogs.value.map((d) => h(d.component, { key: d.id })));
    },
});
beforeEach(() => {
...
Cypress.on('uncaught:exception', (err) => err.message.includes('ResizeObserver loop') ? false : undefined);
describe('dialog.ts — imperative API', () => {
    ...
    it('ignores repeated confirm clicks while a previous call is pending', () => {
        ...
        const onConfirm = () => {
            calls++;
            return new Promise((r) => (resolveOuter = r));
        };
        cy.mount(DialogManager);
        cy.then(() => {
            confirm({ title: 't', onConfirm });
    ...
    it('onConfirm can imperatively call setError to surface a message and unlock buttons', () => {
        let capturedSetError = null;
        const onConfirm = (ctx) => new Promise(() => {
                ...
                fields: [
                    {
                        ...
                        label: 'Email',
                        defaultValue: 'bad',
                        validate: (v) => (v === 'bad' ? 'Invalid email' : null),
                    },
                ],
                onConfirm,
            });
