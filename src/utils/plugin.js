...
const removedOptions = {
    config: 'Call setConfig(key, value) for each entry instead — it is the only entry point now.',
    call: 'The $call global is gone. Import { call } from "frappe-ui" where you need it.',
    socketio: 'initSocket is gone. Create your own socket.io connection and provide it however your app prefers.',
};
...
const removedGlobals = {
    $socket: 'The FrappeUI plugin used to open a socket.io connection and assign it here. ' +
    ...
    $call: 'The FrappeUI plugin used to assign the `call` helper here. It no longer does. ' +
        'Import it instead: import { call } from "frappe-ui".',
};
const RESOURCES_OFF_ADVICE = 'The v1 resources Options API no longer installs by default. ' +
...
function guardRemovedGlobal(app, key, advice) {
        ...
        app.mixin({
            created() {
                if (!this.$options.resources)
                    return;
                const name = this.$options.name || this.$.type?.__name || 'An anonymous component';
                throw new Error(`[frappe-ui] ${name} declares a "resources" component option, but the ` +
