...
describe('<ThemeSwitcher />', () => {
    ...
    it('warns that ThemeSwitcher is deprecated', () => {
        ...
        cy.get('@consoleWarn').should('have.been.calledWithMatch', /ThemeSwitcher is deprecated.*Select.*useColorScheme/);
    });
    ...
    it('marks the bound theme as checked (controlled)', () => {
        ...
        cy.get('[data-theme-option="dark"]').should('have.attr', 'data-state', 'checked');
        cy.get('[data-theme-option="light"]').should('have.attr', 'data-state', 'unchecked');
    });
    it('reflects the selection through aria-checked', () => {
        cy.mount(ThemeSwitcher, { props: { modelValue: 'dark' } });
        cy.get('[data-theme-option="dark"]').should('have.attr', 'aria-checked', 'true');
        cy.get('[data-theme-option="light"]').should('have.attr', 'aria-checked', 'false');
        cy.get('[data-theme-option="system"]').should('have.attr', 'aria-checked', 'false');
    });
