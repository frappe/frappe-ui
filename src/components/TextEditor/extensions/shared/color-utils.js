 ...
 */
export function getClosestNamedColor(color, allowedColors, colorMap, legacyMap) {
 ...
 */
export function extractColorFromStyle(style, allowedColors, colorMap = textColorHexMap, legacyMap = legacyTextColorMap, property = 'color') {
    ...
    if (hexColorMatch && hexColorMatch[1]) {
        const closestOverallColor = getClosestNamedColor(hexColorMatch[1], allColorsInMap, // Search in all colors from the map
        colorMap, legacyMap);
    ...
    const rgbColorMatch = new RegExp(`${property}:\\s*(rgb\\(\\s*\\d+\\s*,\\s*\\d+\\s*,\\s*\\d+\\s*\\))`, 'i').exec(style);
    if (rgbColorMatch && rgbColorMatch[1]) {
        const closestOverallColor = getClosestNamedColor(rgbColorMatch[1], allColorsInMap, // Search in all colors from the map
        colorMap, legacyMap);
        if (closestOverallColor) {
            // Check if this color is in the originally passed 'allowedColors'
            if (allowedColors.includes(closestOverallColor)) {
 ...
 */
export function extractTextColorFromStyle(style, allowedColors) {
 ...
 */
export function extractHighlightColorFromStyle(style, allowedColors) {
    return extractColorFromStyle(style, allowedColors, highlightColorHexMap, legacyHighlightColorMap, 'background-color');
}
