<template>
  <NodeViewWrapper class="table-of-contents-node group relative" contenteditable="false">
    <p v-if="anchorTree.length === 0" class="py-3 text-sm text-ink-gray-5">
    ...
    <button
      v-if="isEditable"
      @click="removeNode"
      class="absolute right-2 top-2 rounded bg-black/65 p-1 text-ink-gray-4 opacity-0 transition-opacity hover:bg-black/80 hover:text-ink-white group-hover:opacity-100"
