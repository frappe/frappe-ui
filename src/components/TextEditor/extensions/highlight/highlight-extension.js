...
import { getClosestNamedColor, highlightColorHexMap, legacyHighlightColorMap, extractHighlightColorFromStyle, } from '../shared/color-utils';
/**
...
export const NamedHighlightExtension = Mark.create({
        ...
        return {
            color: {
                ...
                parseHTML: (element) => {
                    ...
                    if (legacyColorAttr) {
                        const closestColor = getClosestNamedColor(legacyColorAttr, this.options.colors, highlightColorHexMap, legacyHighlightColorMap);
                        if (closestColor) {
                            return closestColor;
                        }
                    }
    ...
    addCommands() {
        return {
            setHighlightByName: (colorName) => ({ chain, commands, editor, state }) => {
            ...
            toggleHighlightByName: (colorName) => ({ chain, commands, editor, state }) => {
                // Validate that the color name is allowed
                if (!this.options.colors.includes(colorName)) {
                    console.warn(`Highlight color "${colorName}" is not in the allowed colors list`);
                    return false;
