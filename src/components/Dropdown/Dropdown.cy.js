...
describe('Dropdown', () => {
    ...
    it('keeps custom item slots as the outer menuitem element', () => {
        cy.mount(Dropdown, {
            ...
            slots: {
                item: ({ item }) => h('button', { 'data-cy': 'custom-item' }, item.label),
            },
        });
        cy.get('[aria-haspopup=menu]').click();
    ...
    it('lets a pointerless click through after a canceled press', () => {
        ...
        cy.get('[aria-haspopup=menu]').then(($trigger) => {
            $trigger[0].dispatchEvent(new MouseEvent('click', { bubbles: true, detail: 0 }));
        });
        cy.get('[role=menu]').should('exist');
    });
    ...
    it('disabled items do not fire onClick and are skipped by the keyboard', () => {
        ...
        cy.get('[aria-haspopup=menu]').click();
        cy.contains('[role=menuitem]', 'Disabled').should('have.attr', 'data-disabled');
        ...
        cy.contains('[role=menuitem]', 'Enabled').should('have.attr', 'data-highlighted');
        cy.get('[role=menu]').type('{downarrow}');
        cy.contains('[role=menuitem]', 'Enabled').should('have.attr', 'data-highlighted');
    });
    it('supports arrow keys, Enter, and Escape', () => {
        ...
        cy.contains('[role=menuitem]', 'Delete').should('have.attr', 'data-highlighted');
    ...
    it('renders group labels, and #group-label overrides them', () => {
        ...
        cy.mount(Dropdown, {
            ...
            slots: {
                'group-label': ({ group }) => h('span', { 'data-cy': 'group-label' }, group.group.toUpperCase()),
            },
        });
        cy.get('[aria-haspopup=menu]').click();
    ...
    it('does not render the removed { group, items } shape', () => {
        cy.mount(Dropdown, {
            props: {
                options: [{ group: 'Edit', items: [{ label: 'Rename' }] }],
            },
        });
        cy.get('[aria-haspopup=menu]').click();
