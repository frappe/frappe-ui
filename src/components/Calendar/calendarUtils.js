...
export function parseDateEventPopupFormat(date, showDay = true, showMonth = true, weekDay = 'short') {
    const options = {
        day: 'numeric',
    };
    if (showMonth) {
...
export function findOverlappingEventsCount(events) {
    ...
    for (const event of events) {
        const availableHall = result.find((hall) => (hall[hall.length - 1].endTime || 0) <= (event.startTime || 0));
        if (availableHall) {
    ...
    return result
        .map((hall, idx) => hall.map((event, eventIdx) => ({
        ...event,
        hallNumber: idx,
        idx: eventIdx,
    })))
...
export const colorMapDark = colorMap;
