...
import { CALENDAR_ACTIONS_KEY, CALENDAR_CONFIG_KEY, } from './types';
const legacyColorNamesByHex = {
    '#db7706': 'amber',
    '#6846e3': 'violet',
    '#e34aa6': 'pink',
    '#3bbde5': 'cyan',
...
export function useEventBase(props) {
    ...
    if (!config || !calendarActions) {
        throw new Error('Calendar event components must be rendered inside Calendar.');
    }
    ...
    function registerDeleteShortcut() {
        if (!config.isEditMode || !config.enableShortcuts)
            return;
