...
function buildParsedTime(hh24, mm, ss) {
...
function parseTimeWithFormat(input, format) {
    ...
    if (!parsed)
        return { valid: false };
    return buildParsedTime(parsed.format('HH'), parsed.format('mm'), formatHasSeconds(timeFormat) ? parsed.format('ss') : undefined);
}
/**
 ...
 */
export function parseFlexibleTime(input, format = DEFAULT_TIME_FORMAT) {
    if (!input)
        return { valid: false };
...
export function normalize24(raw, format = DEFAULT_TIME_FORMAT) {
    if (!raw)
        return '';
    if (/^\d{2}:\d{2}$/.test(raw))
        return raw;
...
export function formatTime(val24, format = DEFAULT_TIME_FORMAT) {
    if (!val24)
        return '';
    const canonicalFormat = val24.length === 8 ? 'HH:mm:ss' : 'HH:mm';
    const parsed = dayjs(`${REFERENCE_DATE} ${val24}`, `${REFERENCE_DATE_FORMAT} ${canonicalFormat}`, true);
    if (!parsed.isValid())
        return val24;
...
export function isOutOfRange(totalMinutes, minMinutes, maxMinutes) {
    if (minMinutes != null && totalMinutes < minMinutes)
        return true;
 ...
 */
export function generateTimeOptions({ interval, format, minMinutes, maxMinutes, }) {
    const out = [];
    for (let m = 0; m < 1440; m += interval) {
        if (isOutOfRange(m, minMinutes, maxMinutes))
            continue;
...
export function findNearestIndex(targetMinutes, options) {
    if (!options.length)
        return -1;
    ...
    const candidates = [];
    if (lo < minutes.length)
        candidates.push(lo);
    if (lo - 1 >= 0)
        candidates.push(lo - 1);
    if (!candidates.length)
        return -1;
    return candidates.sort((a, b) => Math.abs(minutes[a] - targetMinutes) -
        Math.abs(minutes[b] - targetMinutes))[0];
}
