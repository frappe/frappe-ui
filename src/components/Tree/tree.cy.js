...
describe('Tree', () => {
    ...
    it('expands late-arriving nodes while the switch is on', () => {
        const data = ref([]);
        cy.mount({
            render: () => h(Tree, { nodes: data.value, nodeKey: 'id', expanded: true }),
        });
        cy.contains('Node A').should('not.exist');
        cy.then(() => {
    ...
    it('exposes ARIA tree semantics', () => {
        ...
        cy.contains('[role="treeitem"]', 'Node A-1').should('have.attr', 'aria-level', '3');
    });
    ...
    it('renders custom item-label and item-prefix/item-suffix slots', () => {
        cy.mount(Tree, {
            props: { nodes: makeNodes(), nodeKey: 'id', expanded: true },
            slots: {
                'item-label': ({ node }) => h('span', { 'data-cy': `label-${node.id}` }, `label-${node.label}`),
                'item-suffix': ({ node }) => h('span', { 'data-cy': `suffix-${node.id}` }, '★'),
            },
        });
    ...
    describe('drag and drop', () => {
        function dndProps(onDragEnd, extra = {}) {
            return {
                nodes: makeNodes(),
                nodeKey: 'id',
                expanded: true,
        ...
        function dragOnto(sourceLabel, targetLabel, zone) {
        ...
        it('emits drag-end with DropInfo on a valid reparent', () => {
            ...
            cy.get('@dragEnd').then((stub) => {
                const info = stub.firstCall.args[0];
