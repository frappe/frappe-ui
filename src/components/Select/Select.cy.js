...
describe('Select', () => {
    ...
    it('#item replaces the entire option row', () => {
        cy.mount(Select, {
            ...
            slots: {
                item: ({ item, selected }) => h('div', { 'data-cy': 'custom-item', 'data-selected': String(selected) }, item.label),
            },
        });
    ...
    it('a per-option slot wins over the generic #item slot', () => {
        cy.mount(Select, {
            ...
            slots: {
                ...
                'item-special': () => h('div', { 'data-cy': 'special-item' }, 'special'),
            },
        });
        cy.get('[role=combobox]').click();
        ...
        const Harness = defineComponent({
            setup() {
                ...
                return () => h('div', [
                    ...
                    h('button', { 'data-cy': 'clear', onClick: () => selectRef.value?.clear() }, 'Clear'),
                    h('button', {
                        'data-cy': 'focus',
    ...
    it('#footer receives the control slot props', () => {
        cy.mount(Select, {
            props: { options, modelValue: 'def' },
            slots: {
                footer: ({ selectedOption, clear, setOpen }) => h('div', [
                    h('span', { 'data-cy': 'footer-label' }, selectedOption?.label),
                    h('button', { 'data-cy': 'footer-clear', onClick: clear }, 'Clear'),
                    h('button', { 'data-cy': 'footer-close', onClick: () => setOpen(false) }, 'Close'),
                ]),
            },
        });
        cy.get('[role=combobox]').click();
    ...
    it('sizes the trigger to the selected value, not the widest option', () => {
        ...
        const WidthHarness = defineComponent({
            setup() {
                return () => h('div', { class: 'flex items-start gap-6' }, [
                    h(Select, {
    ...
    it('renders full width only when width is explicitly set', () => {
        const WidthHarness = defineComponent({
            setup() {
                return () => h('div', { class: 'w-[320px] space-y-4' }, [
                    h(Select, {
                        options,
                        modelValue: 'abc',
    ...
    it('keeps item-aligned positioning when using a custom trigger slot', () => {
        cy.mount(Select, {
            ...
            slots: {
                trigger: ({ selectedOption }) => h('div', { 'data-cy': 'trigger-content' }, selectedOption?.label || 'Pick one'),
            },
        });
    ...
    it('opens without an enter/exit animation (instant motion)', () => {
        ...
        cy.get('[role=combobox]').click();
        cy.get('[data-slot="content-body"]').should('have.attr', 'data-motion', 'instant');
    });
    ...
    describe('shared labeling contract', () => {
        ...
        it('renders required indicator and forwards aria-required', () => {
            ...
            cy.get('[data-slot="trigger"]').should('have.attr', 'aria-required', 'true');
        ...
        it('flips data-invalid on the trigger when error is set', () => {
            ...
            cy.get('[data-slot="trigger"]').should('have.attr', 'data-invalid', 'true');
        });
