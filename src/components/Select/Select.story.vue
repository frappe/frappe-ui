...
<template>
  <Story title="Select" :layout="{ type: 'grid', width: 400 }">
    ...
    <Variant title="Disabled State">
      <div class="p-4">
        <label class="block text-sm font-medium mb-2">Disabled Select</label>
        <Select
          :options="simpleOptions"
    ...
    <Variant title="Change Display Value">
      <div class="p-4">
        <label class="block text-sm font-medium mb-2"
          >Multiple Grouped Options</label
        >
        <Select
          ...
          :get-label="
            (val, selected) => {
              if (!selected) return val.label
              return !val.length
                ? 'Choose a fruit!'
                : 'A collection of delicious fruit.'
            }
          "
        />
        <div class="mt-2 text-sm text-gray-600">
