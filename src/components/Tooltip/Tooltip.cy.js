...
import { h } from 'vue';
const Comp = () => h(Tooltip, { text: 'some tooltip', hoverDelay: 0, side: 'top' }, [
    h(Button, {}, 'k'),
]);
describe('Tooltip', () => {
    it('Renders', () => {
    ...
    it('offset moves the bubble away from the trigger', () => {
        cy.mount(h(Tooltip, { text: 'abc', hoverDelay: 0, offset: 24 }, [
            h(Button, {}, 'k'),
        ]));
        cy.get('button').trigger('focus');
    ...
    it('grouping under a shared TooltipProvider skips the re-delay between triggers', () => {
        ...
        const Group = () => h(TooltipProvider, { hoverDelay: 0.5, skipDelay: 0.3, disableHoverableContent: true }, () => [
            h(Tooltip, { text: 'First' }, () => h('button', { 'data-cy': 'first' }, 'one')),
            h(Tooltip, { text: 'Second' }, () => h('button', { 'data-cy': 'second' }, 'two')),
        ]);
        cy.clock();
        cy.mount(Group);
