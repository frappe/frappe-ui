...
describe('DatePicker', () => {
    ...
    it('action btns', () => {
        ...
        cy.get('[aria-label=cycle-calendar-view]').should('have.text', monthsLabels[prevMonthIndex] + ' ' + prevYear);
        cy.get('[aria-label=next]').dblclick();
        cy.get('[aria-label=cycle-calendar-view]').should('have.text', monthsLabels[nextMonthIndex] + ' ' + nextYear);
    });
    ...
    it('isDateUnavailable callback disables matching cells', () => {
        cy.mount(DatePicker, {
            props: {
                ...
                isDateUnavailable: (d) => d.day() === 0 || d.day() === 6,
            },
        });
        cy.get('input').dblclick();
        ...
        it('arrow navigation skips disabled dates', () => {
            cy.mount(DatePicker, {
                props: {
                    ...
                    isDateUnavailable: (d) => d.format('YYYY-MM-DD') === '2025-06-16',
                },
            });
            cy.get('input').focus().type('{downArrow}');
            cy.focused().trigger('keydown', { key: 'ArrowRight' });
