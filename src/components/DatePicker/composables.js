...
export function usePopoverPositioning(props, legacy) {
    const resolvedSide = computed(() => props.side ?? (legacy.placement?.split('-')[0] ?? 'bottom'));
    const resolvedAlign = computed(() => {
        if (props.align !== undefined)
            return props.align;
        const fromPlacement = legacy.placement?.split('-')[1];
...
export function useTypeable(props, legacy) {
    return computed(() => props.typeable === false ||
...
export function useKeepOpen(props, legacy) {
...
export function useDateCoercion(getFormat) {
    return function coerceToDayjs(val) {
        if (!val)
            return null;
...
export function makeUnavailableCheck(getMin, getMax, getIsUnavailable) {
    return function checkUnavailable(d) {
        const min = getMin();
        const max = getMax();
        const isUnavailable = getIsUnavailable();
        if (min && d.isBefore(dayjs(min), 'day'))
            return true;
        if (max && d.isAfter(dayjs(max), 'day'))
            return true;
        if (isUnavailable?.(d))
            return true;
        return false;
...
export function useDeprecationWarnings(componentName, legacy, options = {}) {
    ...
    const targetSlotPresent = () => typeof hasTargetSlot === 'object' && hasTargetSlot !== null
        ? hasTargetSlot.value
        : !!hasTargetSlot;
    watchEffect(() => {
        if (targetSlotPresent() && !warned.targetSlot) {
            console.warn(`[${componentName}] \`#target\` slot is deprecated. Use \`#trigger\` instead.`);
            warned.targetSlot = true;
        }
        if (legacy.value && !warned.value) {
            console.warn(`[${componentName}] \`value\` is deprecated. Use \`v-model\` / \`modelValue\` instead.`);
            warned.value = true;
        }
        if (legacy.placement !== undefined && !warned.placement) {
            console.warn(`[${componentName}] \`placement\` is deprecated. Use \`side\` and \`align\` instead.`);
            warned.placement = true;
        }
        if (legacy.autoClose === false && !warned.autoClose) {
            console.warn(`[${componentName}] \`autoClose: false\` is deprecated. Use \`keepOpen: true\` instead.`);
            warned.autoClose = true;
        }
        if (legacy.allowCustom === false && !warned.allowCustom) {
            console.warn(`[${componentName}] \`allowCustom: false\` is deprecated. Use \`typeable: false\` instead.`);
            warned.allowCustom = true;
        }
        if (legacy.readonly === true && !warned.readonly) {
            console.warn(`[${componentName}] picker-level \`readonly: true\` is deprecated. Use \`typeable: false\` instead.`);
            warned.readonly = true;
        }
        if (legacy.inputClass && !warned.inputClass) {
            console.warn(`[${componentName}] \`inputClass\` is deprecated. Apply \`class\` directly to the component element to control width.`);
            warned.inputClass = true;
        }
        if (legacy.minDateTime && !warned.minDateTime) {
            console.warn(`[${componentName}] \`minDateTime\` is deprecated. Use \`min\` instead.`);
            warned.minDateTime = true;
        }
        if (legacy.maxDateTime && !warned.maxDateTime) {
            console.warn(`[${componentName}] \`maxDateTime\` is deprecated. Use \`max\` instead.`);
