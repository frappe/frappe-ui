...
describe('Dialog', () => {
    ...
    it('renders title, message and action; ctx.close() closes the dialog', () => {
        ...
        const Wrapper = defineComponent({
            ...
            render() {
                return [
                    h(Button, { onClick: () => (this.open = true) }, { default: () => 'Show' }),
                    h(Dialog, {
                        open: this.open,
                        'onUpdate:open': (v) => (this.open = v),
                        onClose,
                        title: 'Modal Dialog',
    ...
    it('focuses and types into an input clicked inside a scrollable dialog', () => {
        ...
        cy.get('.dialog-scroll-container').should(($scroller) => {
            expect($scroller[0].scrollHeight).to.be.greaterThan($scroller[0].clientHeight);
        });
    ...
    it('bare suppresses chrome and renders only the default slot', () => {
        cy.mount(Dialog, {
            ...
            slots: {
                default: h('div', { 'data-cy': 'bare-content', class: 'p-6' }, 'bare content'),
            },
        });
        cy.get('[role=dialog]').should('exist');
        ...
        const Wrapper = defineComponent({
            ...
            render() {
                return h(Dialog, {
                    ...
                    title: ({ close }) => h('button', { 'data-cy': 'title-close', onClick: close }, 'Custom title'),
                    default: ({ close }) => h('button', { 'data-cy': 'default-close', onClick: close }, 'Custom body'),
                    actions: ({ close, actions, }) => [
                        h('div', { 'data-cy': 'actions-relayout' }, actions.map((a) => a.label).join(', ')),
    ...
    it('focuses a descendant marked with `autofocus` on open', () => {
        ...
        cy.window().then((win) => {
            const el = win.document.querySelector('[data-cy=name-input]');
