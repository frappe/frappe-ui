...
describe('Rating', () => {
    it('Renders', () => {
        ...
        cy.get('[data-slot="star"]').each(($el, i) => {
            cy.wrap($el).should('have.attr', 'data-state', i < 2 ? 'filled' : 'empty');
        });
    });
    it('v-model', () => {
    ...
    describe('clearing', () => {
        ...
        it('consumer can suppress clearing by filtering the 0 update', () => {
            ...
            cy.mount(wrapper);
            cy.get('[role="radio"]').eq(2).click();
            cy.get('[role="radio"][aria-checked="true"]').should('have.attr', 'data-index', '3');
        });
    });
    describe('icon', () => {
        ...
        it('icon slot receives per-half state under step=0.5', () => {
            cy.mount(Rating, {
                ...
                slots: {
                    icon: ({ index, side, state, }) => h('i', { 'data-test': `s${index}-${side}`, 'data-state': state }),
                },
            });
