...
describe('Popover', () => {
    ...
    describe('contract', () => {
        ...
        it('exposes reactive open state to the #trigger slot', () => {
            ...
            cy.mount(Popover, {
                slots: {
                    trigger: ({ open }) => h(Button, { 'data-cy': 'trigger' }, () => open ? 'Close' : 'Open'),
        ...
        it('exposes close() to the #default slot content', () => {
            cy.mount(Popover, {
                slots: {
                    ...
                    default: ({ close }) => h(Button, { 'data-cy': 'dismiss', onClick: () => close() }, () => 'Done'),
                },
            });
            cy.get('[data-cy="trigger"]').click();
            cy.get('[data-slot="content"]').should('exist');
        ...
        it('supports v-model:open (controlled open/close)', () => {
            const Harness = defineComponent({
                setup() {
                    ...
                    return () => h('div', [
                        h(Button, { 'data-cy': 'external', onClick: () => (open.value = true) }, () => 'Open'),
                        h(Popover, {
                            open: open.value,
        ...
        it('exposes open() and close() methods', () => {
            ...
            const Harness = defineComponent({
                setup() {
                    return () => h(Popover, { ref: (el) => (popoverRef.value = el) }, {
                        trigger: () => h(Button, { 'data-cy': 'trigger' }, () => 'T'),
