...
export default function useEchartsOptions(config) {
              ...
              `;
                ...
                if (Array.isArray(params)) {
                    const t = params.map((p, idx) => {
                        ...
                        const formattedX = config.xAxis.type == 'time'
...
export function getTitleOptions(title, subtitle, isRTL) {
    return {
        top: '4px',
...
function getYAxisOptions(config) {
    ...
    primaryYAxisOptions = mergeDeep(primaryYAxisOptions, config.swapXY ? config.xAxis.echartOptions : config.yAxis.echartOptions);
    ...
    secondaryYAxisOptions = mergeDeep(secondaryYAxisOptions, config.swapXY ? config.y2Axis?.echartOptions : config.y2Axis?.echartOptions);
