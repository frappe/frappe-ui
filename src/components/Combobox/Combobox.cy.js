...
describe('Combobox', () => {
    ...
    describe('selection', () => {
        ...
        it('marks the selected item via data-state=checked', () => {
            ...
            cy.contains('[role="option"]', 'Mango').should('have.attr', 'data-state', 'checked');
        });
    ...
    describe('query ownership', () => {
        ...
        it('keeps a seeded v-model:query when the popover opens, and filters with it', () => {
            ...
            cy.get('[data-slot="search"] [data-slot="input"]').should('have.value', 'ma');
        ...
        it('still resets the query on open when it is not bound (button mode)', () => {
            ...
            cy.get('[data-slot="content"]').should('not.exist');
            cy.get('[data-slot="trigger"]').click();
            cy.get('[data-slot="search"] [data-slot="input"]').should('have.value', '');
            cy.get('[role="option"]').should('have.length', 3);
        });
            ...
            const InputMode = defineComponent({
                ...
                render() {
                    return h('div', [
                        ...
                        h('button', {
                            'data-cy': 'clear',
                            onClick: () => this.$refs.picker?.clear(),
                        }, 'clear'),
            ...
            const ButtonMode = defineComponent({
                ...
                render() {
                    return h(Combobox, {
                        ...
                        footer: ({ clear }) => h('button', { 'data-cy': 'clear', onClick: clear }, 'clear'),
                    });
                },
            });
            cy.mount(ButtonMode);
    ...
    describe('highlighting', () => {
        it('re-highlights the first item when options are replaced after typing', () => {
            ...
            const AsyncSearch = defineComponent({
                setup() {
                    const options = ref(['Stale result']);
                    return { options };
                },
                render() {
            ...
            cy.mount(AsyncSearch).then((mounted) => {
    ...
    describe('create-new via a custom row', () => {
        const withCreateRow = (onCreate) => ({
            options: [
                ...fruits,
                {
                    ...
                    onClick: ({ query }) => onCreate(query.trim()),
                },
            ],
            openOnFocus: true,
        });
        it('clicking the row commits the typed query', () => {
            cy.mount(Combobox, {
                ...
                slots: { 'item-create': ({ query }) => `Create "${query}"` },
            });
    ...
    describe('grouped options', () => {
        it('renders grouped options and omits empty groups', () => {
            cy.mount(Combobox, {
                props: {
                    open: true,
                    options: [
                        { group: 'Fruit', options: ['Apple', null, 'Mango'] },
                        { group: 'Empty', options: [null] },
                    ],
                },
            });
        ...
        it('uses #group-label slot override', () => {
            cy.mount(Combobox, {
                ...
                slots: {
                    'group-label': ({ group }) => h('span', { 'data-cy': 'group' }, `→ ${group.group}`),
                },
            });
                    ...
                    options: [
                        {
                            ...
                            slots: {
                                ...
                                label: ({ item }) => h('span', { 'data-cy': 'l' }, item.label),
    ...
    describe('namespaced slot dispatch', () => {
        it('dispatches `option.slot` to the matching `#item-<slot>` template slot', () => {
            cy.mount(Combobox, {
                ...
                slots: {
                    'item-person': ({ item }) => h('div', { 'data-cy': 'person-slot' }, item.label),
                    'item-create': ({ item }) => h('div', { 'data-cy': 'create-slot' }, item.label),
                },
            });
    ...
    describe('custom options', () => {
        it('onClick receives { query } context and closes by default', () => {
            ...
            cy.get('@onCustomClick').should('have.been.calledWithMatch', Cypress.sinon.match({ query: 'foo' }));
            cy.get('@onOpen').should('have.been.calledWith', false);
        });
    ...
    describe('empty, loading, and footer', () => {
        ...
        it('renders custom #empty slot with the current query', () => {
            cy.mount(Combobox, {
                ...
                slots: {
                    empty: ({ query }) => h('div', { 'data-cy': 'empty' }, `No results for ${query}`),
                },
            });
        ...
        it('renders #footer slot with control props', () => {
            cy.mount(Combobox, {
                ...
                slots: {
                    footer: ({ setOpen }) => h('button', { 'data-cy': 'footer', onClick: () => setOpen(false) }, 'FOOTER'),
                },
            });
    ...
    describe('custom trigger (button mode)', () => {
        it('renders the #trigger slot in place of the input, and moves search into the popover', () => {
            cy.mount(Combobox, {
                props: { options: fruits, open: true },
                slots: {
                    trigger: ({ displayValue }) => h('button', { type: 'button', 'data-cy': 'custom-btn' }, displayValue || 'Pick a fruit'),
                },
            });
        ...
        it('focuses the popover search input on open and filters from there', () => {
            cy.mount(Combobox, {
                ...
                slots: {
                    trigger: () => h('button', { type: 'button', 'data-cy': 'btn' }, 'Pick'),
                },
            });
    ...
    describe('search row (button mode)', () => {
        ...
        it('search slot props expose { query, setQuery, disabled, focus }', () => {
            ...
            cy.get('[data-cy="set-query"]').should('have.attr', 'data-disabled', 'false');
    ...
    describe('imperative API', () => {
        ...
        it('exposes `focus()` which focuses the input (input mode)', () => {
            cy.mount(Combobox, { props: { options: fruits } }).then((mounted) => {
                const vm = mounted.component ?? mounted.wrapper?.vm ?? mounted;
                vm?.focus?.();
            });
    ...
    describe('inside a Dialog', () => {
        it('focuses and accepts typing in the popover search input (button mode)', () => {
            const Wrapper = defineComponent({
                ...
                render() {
                    return h(Dialog, { open: this.open, title: 'Pick' }, {
                        default: () => h(Combobox, {
                            options: fruits,
                            trigger: 'button',
                            placeholder: 'Pick',
