...
describe('MultiSelect', () => {
    ...
    it('renders search slots with scoped query controls', () => {
        cy.mount(MultiSelect, {
            ...
            slots: {
                'search-prefix': ({ query }) => h('span', { 'data-cy': 'search-prefix' }, query || 'Search'),
                'search-suffix': ({ query, setQuery, focus, disabled }) => h('div', { 'data-cy': 'search-suffix' }, [
        ...
        cy.get('[data-slot="trigger"]').click();
        cy.get('[data-slot="search"] [data-cy="search-prefix"]').should('contain.text', 'Search');
    ...
    describe('inside a Dialog', () => {
        it('focuses and accepts typing in the popover search input', () => {
            const Wrapper = defineComponent({
                ...
                render() {
                    return h(Dialog, { open: this.open, title: 'Pick fruits' }, { default: () => h(MultiSelect, { options }) });
                },
            });
            cy.mount(Wrapper);
    ...
    describe('shared labeling contract', () => {
        ...
        it('renders description and wires aria-describedby on trigger', () => {
            ...
            cy.get('[data-slot="trigger"]').then(($trigger) => {
                ...
                cy.get(`#${id}-description`).should('contain.text', 'Pick as many as you like.');
            });
        });
