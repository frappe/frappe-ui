...
const panel = (text) => defineComponent({ render: () => h('div', { class: 'tab-panel' }, text) });
const tabs = [
...
const Harness = defineComponent({
    ...
    setup(props, { emit }) {
        ...
        return () => h(SettingsDialog, {
            ...
            tab: active.value,
            'onUpdate:tab': (v) => (active.value = v),
        }, () => [
            h(SettingsSidebar, () => h(SettingsNavGroup, { label: 'Workspace' }, () => tabs.map((tab) => h(SettingsNavItem, { key: tab.value, value: tab.value }, () => tab.label)))),
            h(SettingsContent, () => tabs.map((tab) => h(SettingsPanel, { key: tab.value, value: tab.value }, () => h(tab.component)))),
        ]);
    },
});
describe('SettingsDialog', () => {
    ...
    it('exposes tablist / tab / tabpanel roles with aria-selected', () => {
        ...
        cy.contains('[role=tab]', 'Profile').should('have.attr', 'aria-selected', 'true');
        cy.contains('[role=tab]', 'Members').should('have.attr', 'aria-selected', 'false');
