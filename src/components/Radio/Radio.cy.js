...
function mountGroup(groupProps = {}, options = [
...
describe('Radio', () => {
    ...
    it('throws when used outside a RadioGroup', () => {
        ...
        cy.get('@onError').should('have.been.calledWithMatch', /must be used inside a <RadioGroup>/);
    });
        ...
        it('looks disabled, not just behaves disabled, when the group is', () => {
            ...
            cy.get('[role="radio"]')
                ...
                cy.get('[role="radio"]')
                    .first()
                    .find('[data-slot="label"]')
                    .should('not.have.css', 'color', disabledColor);
            });
        });
    ...
    describe('alignment', () => {
        ...
        function expectCentredOnFirstLine() {
            cy.get('[role="radio"]')
                ...
                expect(circle.top + circle.height / 2).to.be.closeTo(labelRect.top + lineHeight / 2, 0.5);
            });
        }
    ...
    describe('orientation', () => {
        it('lays options out in a row when horizontal', () => {
            ...
            cy.get('[role="radiogroup"]').should('have.attr', 'data-orientation', 'horizontal');
        });
    });
});
