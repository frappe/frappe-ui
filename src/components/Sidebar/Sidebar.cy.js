...
describe('<Sidebar /> legacy config API (compat layer)', () => {
    ...
    it('still renders header, items, and the collapse toggle from config props', () => {
        ...
        cy.mount(() => h('div', [h(Sidebar, { header, sections, class: 'w-fit' })]), {
    ...
    it('passes legacy header-logo and footer-items slots through', () => {
        cy.viewport(1280, 720);
        cy.mount(() => h('div', h(Sidebar, { header, sections, disableCollapse: true }, {
            'header-logo': () => h('span', { 'data-test': 'legacy-logo' }, 'L'),
            'footer-items': () => h('button', { 'data-test': 'legacy-footer' }, 'Invite'),
        })), { global: { plugins: [createTestRouter()] } });
...
describe('<SidebarItem />', () => {
    ...
    it('renders #prefix, default, and #suffix slots', () => {
        ...
        cy.get('[data-slot=sidebar-item-suffix] [data-test=suffix]').should('contain.text', '3');
    });
    it('keeps a #suffix options button a sibling of the link (not nested inside it)', () => {
        ...
        cy.mount(SidebarItem, {
            ...
            slots: {
                suffix: () => h('button', { 'data-test': 'options', onClick: onOptions }, '...'),
            },
            global: { plugins: [createTestRouter()] },
        });
