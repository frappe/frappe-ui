...
describe('<Tag />', () => {
  it('renders a static gray subtle md span by default', () => {
    cy.mount(Tag, { props: { label: 'Discover' } })
    cy.get(root)
      .should('have.prop', 'tagName', 'SPAN')
      .and('not.have.class', 'hover:bg-surface-gray-3')
      ...
      .and('have.class', 'h-6')
    cy.get(root).should('not.have.attr', 'tabindex')
    cy.get('[data-slot="dismiss"]').should('not.exist')
  })

  it('renders a button once it has a click listener or is dismissible', () => {
    cy.mount(Tag, { props: { label: 'x', onClick: () => {} } })
    cy.get(root)
      .should('match', 'button[type="button"]')
      .and('have.class', 'hover:bg-surface-gray-3')
    cy.mount(Tag, { props: { label: 'x', dismissible: true } })
    cy.get(root).should('match', 'button[type="button"]')
  })

  it('exposes focus()', () => {
    cy.mount(Tag, { props: { label: 'x', dismissible: true } }).then(
      ({ component }) => {
        ;(component as any).focus()
      },
    )
    cy.focused().should('have.attr', 'data-slot', 'tag')
  })

  it('describes the Delete shortcut to screen readers', () => {
    cy.mount(Tag, { props: { label: 'x', dismissible: true } })
    cy.get(root)
      .invoke('attr', 'aria-describedby')
      .then((id) => {
        cy.get(`#${CSS.escape(id as string)}`).should(
          'contain.text',
          'Press Delete to remove',
        )
      })
    cy.get('[data-slot="dismiss"] .lucide-x').should('exist')
  })

  it('maps each variant to its Figma tokens', () => {
    cy.mount(Tag, {
      props: { label: 'x', variant: 'solid', theme: 'blue', dismissible: true },
    })
    cy.get(root)
  ...
  it('Delete does nothing when the tag is not dismissible', () => {
    const onDismiss = cy.spy().as('onDismiss')
    cy.mount(Tag, { props: { label: 'Open', onDismiss, onClick: () => {} } })
