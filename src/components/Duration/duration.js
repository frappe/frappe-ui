 ...
 */
export function formatDuration(totalSeconds, format = 'short') {
    ...
    if (format === 'short')
        return formatShort(hours, minutes, secs);
    if (format === 'long')
        return formatLong(hours, minutes, secs);
    if (format === 'colon')
        return formatColon(hours, minutes, secs);
...
//   "hh:mm:ss"       → "01:30:45"
// ---------------------------------------------------------------------------
function renderTemplate(template, hours, minutes, secs) {
...
function formatShort(hours, minutes, secs) {
    const parts = [];
    if (hours)
        parts.push(`${hours}h`);
    if (minutes)
...
function formatLong(hours, minutes, secs) {
    const parts = [];
    if (hours)
...
function formatColon(hours, minutes, secs) {
    const pad = (value) => String(value).padStart(2, '0');
    if (hours)
...
function parseUnits(input) {
    ...
    while ((match = tokenRe.exec(input)) !== null) {
        if (input.slice(lastEnd, match.index).trim() !== '')
            return null;
