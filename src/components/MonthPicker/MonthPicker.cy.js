...
describe('MonthPicker', () => {
    ...
    it('warns that MonthPicker is deprecated', () => {
        ...
        cy.get('@consoleWarn').should('have.been.calledWithMatch', /MonthPicker is deprecated.*Select/);
    });
    it('renders', () => {
        cy.mount(MonthPicker);
    ...
    it('action buttons', () => {
        ...
        cy.get('[aria-label="Toggle view mode"]').should('have.text', `${yearRangeStart} - ${yearRangeStart + 11}`);
        cy.get('[aria-label="Toggle view mode"]').click();
        cy.get('[aria-label="Toggle view mode"]').should('have.text', currentYear);
    });
    it('disabled', () => {
