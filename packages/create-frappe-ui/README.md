...
It creates `frontend/` and sets up the Python side:
...
existing `www/*.py` file, and it doesn't add a route rule that is already there,
so it's safe to run again. When `hooks.py` has a shape it can't edit safely, it
prints the rule for you to add.
...
writes `"frappe-ui": "^1.0.0"`. So `npm create frappe-ui@1.2.0` sets up a project
on frappe-ui 1.2.
...
2. Publish frappe-ui, then publish this package from `packages/create-frappe-ui`.
   Give it the `latest` tag, even for a prerelease, because the docs tell people
   to run `npm create frappe-ui@latest`:
