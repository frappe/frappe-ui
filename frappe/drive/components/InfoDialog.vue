<template>
  <Dialog v-model="open" @close="dialogType = ''">
    <template #body-title>
      <h3
        class="cursor-pointer pr-2 text-2xl font-semibold leading-6 text-ink-gray-9"
    ...
    <template #body-content>
      <ul class="mb-4 space-y-3 text-sm text-ink-gray-8">
      ...
      <ul v-if="editor?.storage?.characterCount" class="mb-4 space-y-3 text-sm text-ink-gray-8">
      ...
      <div class="flex items-center justify-between">
        ...
        <Button
          ...
          :variant="'subtle'"
          size="sm"
          class="flex scale-[90%] items-center justify-center rounded"
          @click="emitter.emit('share')"
        >
          {{ __('Manage') }}
        </Button>
      </div>
      <LoadingIndicator v-if="!getGeneralAccess.data?.type" class="mx-auto my-1 size-4" />
      <ul v-else class="space-y-3 py-2 text-sm">
        <li class="flex">
          <span class="inline-block w-24 text-ink-gray-5">{{ __('General') }}:</span>
          <div class="col-span-1 flex gap-2">
            <div class="col-span-1 flex items-center gap-2 text-ink-gray-8">
              <div
                class="flex size-4.5 items-center justify-center rounded-full p-0.5"
      ...
      <ul v-if="developer" class="mb-4 mt-4 space-y-3 text-sm text-ink-gray-8">
        <div>
          <span class="text-base font-semibold">{{ __('Developer') }}</span>
          <Button variant="subtle" size="sm" class="float-right scale-[90%]">
