...
import { getDefaultOperator, getOperators, getValueControl, parseFilters } from './utils'
...
const doctypeFields = createResource({
  ...
  transform(data) {
    const options = data.docs[0].fields
      ...
      .filter((x) => !['Section Break', 'Read Only', 'Column Break'].includes(x.type))
...
const filterCount = computed(() => rows.value.filter((row) => row.field.fieldName !== '').length)
</script>
...
<template>
  ...
  >
    <template #target="{ close, togglePopover }">
      <Button @click="togglePopover()" :class="{ 'rounded-r-none': filterCount != 0 }">
        <template #prefix><FilterIcon /></template>
        Filter
        <Badge
          v-if="filterCount != 0"
          :class="'ml-1 rounded bg-surface-gray-4'"
          :label="filterCount"
        />
      </Button>
      ...
      <Button
        ...
        class="rounded-l-none bg-surface-gray-4"
        icon="x"
      />
    </template>

    <template #body="{ close }">
      <div class="grid items-center gap-2 lg:grid-cols-[1fr_0.7fr_1fr_auto]">
        <!-- input fields -->
        <template v-for="(row, index) in rows">
          ...
          <component :is="getValueControl(row)" v-model="row.value" @update:modelValue="apply" />
          <Button class="ml-auto lg:-ml-1" icon="x" variant="ghost" @click="deleteRow(index)" />
        </template>
      </div>

      <!-- footer buttons -->
      <div class="mt-4 flex justify-between gap-2">
        <Button variant="ghost" @click="insertRow()" icon-left="plus"> Add Filter </Button>
