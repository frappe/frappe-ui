<template>
  <div class="w-[85%] lg:w-[700px] mx-auto py-12 space-y-8 text-base">
    <div class="flex justify-between">
      ...
      <div
        class="flex flex-col lg:flex-row space-y-2 lg:space-x-2 lg:space-y-0"
      >
        <Button
          v-if="mappingUpdated"
          label="Reset Mapping"
        ...
        <Button
          label="Continue"
          variant="solid"
          @click="$emit('updateStep', 'preview')"
        />
      </div>
    </div>

    <div
      v-if="Object.keys(columnMappings).length"
      class="border rounded-md space-y-8"
    >
...
const initializeColumnMappings = () => {
  ...
  if (props.data?.template_options)
    columnToFieldMap = JSON.parse(props.data?.template_options)?.[
      'column_to_field_map'
    ]
...
const getChildTableName = (parent: string, child: string) => {
  ...
  let childField = parentFields.filter(
    (field: any) => field.options == child,
  )[0]
