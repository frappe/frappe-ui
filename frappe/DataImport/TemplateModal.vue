...
const exportType = ref<'All Records' | '5 Records' | 'Blank Template'>(
  'Blank Template',
)
...
const transformFields = (data: any) => {
  let doctypeMap: Record<
    string,
    { fieldname: string; label: string; reqd: number }[]
  > = {}
...
const getExportFields = () => {
  ...
  Object.keys(fieldSelection.value).forEach((doctype: string) => {
    ...
    exportFields[doctypeName] = Object.keys(
      fieldSelection.value[doctype],
    ).filter((fieldname: string) => fieldSelection.value[doctype][fieldname])
  })
  return exportFields
}
