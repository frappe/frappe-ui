<template>
  <div
    class="text-base h-full flex flex-col w-[90%] lg:w-[700px] mx-auto py-12 space-y-10"
  >
    <div class="flex flex-col space-y-1">
      <div class="flex items-center justify-between text-ink-gray-7">
        <div
          class="flex items-center space-x-2 text-lg font-semibold text-ink-gray-9"
        >
    ...
    <div v-if="mapping.length" class="space-y-2">
      ...
      <div
        class="border rounded-md bg-surface-gray-2 p-4 space-y-4 text-sm text-ink-gray-7"
      >
        <div
        ...
        >
          ...
          <div class="flex justify-end">
            <FeatherIcon
              name="arrow-right"
              class="inline size-4 text-ink-gray-5"
            />
          </div>
          <div>
            {{ map[1] }}
    ...
    <div v-if="data.status != 'Pending' && importLogs.length" class="space-y-4">
      ...
      <div class="rounded-md p-2" :class="importBannerClass">
        {{ importSuccessCount }}
        {{ importSuccessCount == 1 ? 'row' : 'rows' }} imported successfully,
        {{ importErrorCount }}
      ...
      <div v-if="filteredLogs.length" class="border rounded-md overflow-x-auto">
        <table class="table-fixed w-full divide-y">
          ...
          <tbody class="bg-surface-white divide-y divide-surface-gray-2">
            <tr
              v-for="(row, rowIndex) in filteredLogs"
              :key="rowIndex"
              class="group"
            >
...
const reloadPreviewData = (dataImport: string) => {
  ...
  nextTick(() => {
    ...
    nextTick(async () => {
      let updatedData = props.dataImports.data?.find(
        (d) => d.name === props.data.name,
      )
...
const warnings = computed(() => {
  return (
    preview.value?.warnings?.filter((warning: any) => warning.type != 'info') ||
    []
  )
})
