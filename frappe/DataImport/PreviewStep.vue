<template>
  <div class="mx-auto flex h-full w-[90%] flex-col space-y-10 py-12 text-base lg:w-[700px]">
    ...
    <div v-if="mapping.length" class="space-y-2">
      <div class="text-sm text-ink-gray-5">Column Mapping</div>
      <div class="space-y-4 rounded-md border bg-surface-gray-2 p-4 text-sm text-ink-gray-7">
        <div
          v-for="map in mapping"
          class="grid grid-cols-[40%,10%,40%] items-center space-x-3 lg:grid-cols-3"
        >
          <div class="">
            {{ map[0] }}
          </div>
    ...
    <div v-if="warnings.length" class="space-y-2">
      <div class="text-sm text-ink-gray-5">Warnings</div>
      <div class="space-y-2 rounded-md bg-surface-amber-2 p-2 text-xs">
    ...
    <div v-if="preview?.data?.length" class="overflow-x-auto rounded-md border">
      <table class="divide-y">
        ...
        <tbody class="divide-surface-gray-2 divide-y bg-surface-white">
          <tr v-for="(row, rowIndex) in previewData" :key="rowIndex">
            <td
              v-for="column in previewColumns"
              :key="column.key"
              :style="{ minWidth: column.width, textAlign: column.align }"
              class="px-3 py-2 align-top text-sm text-ink-gray-7"
              :class="{
                'border-r': column.key != previewColumns[previewColumns.length - 1].key,
              }"
            >
    ...
    <div v-if="data.status != 'Pending' && importLogs.length" class="space-y-4">
      ...
      <div class="rounded-md p-2" :class="importBannerClass">
        ...
        successfully, {{ importErrorCount }} {{ importErrorCount == 1 ? 'row' : 'rows' }} failed.
      </div>

      <TabButtons :buttons="tabButtons" v-model="activeTab" class="w-fit" />

      <div v-if="filteredLogs.length" class="overflow-x-auto rounded-md border">
        <table class="w-full table-fixed divide-y">
          <thead class="rounded-t-md">
            <tr>
              <th class="w-20 border-r p-2 text-left text-center text-sm text-ink-gray-5">
                Row no.
              </th>
              <th class="w-[80%] p-2 text-left text-sm text-ink-gray-5">Message</th>
            </tr>
          </thead>
          <tbody class="divide-surface-gray-2 divide-y bg-surface-white">
            <tr v-for="(row, rowIndex) in filteredLogs" :key="rowIndex" class="group">
              <td class="border-r px-3 py-2 text-sm text-ink-gray-7">
                <div class="flex items-center justify-center space-x-2">
                  <div v-if="row.success" class="size-1.5 rounded bg-surface-green-3"></div>
                  <div v-else class="size-1.5 rounded bg-surface-red-5"></div>
                  <div>
              ...
              <td class="w-full px-3 py-2 text-sm text-ink-gray-7">
                <span v-if="rowMessage(row)" v-html="rowMessage(row)"> </span>
                <span v-else-if="!rowMessage(row) && !row.success"> Failed to import </span>
              ...
              <td class="invisible float-right px-3 py-2 text-ink-gray-5 group-hover:visible">
                <Popover v-if="row.exception" trigger="hover" placement="left-start">
                  <template #target>
                    <FeatherIcon name="info" class="size-4" />
                  </template>
                  <template #body-main>
                    <div class="w-[500px] p-2 font-mono text-xs leading-5">
                      {{ row.exception }}
                    </div>
                  </template>
                </Popover>
      ...
      <div v-else class="text-sm text-ink-gray-5">No logs to display.</div>
    </div>
  </div>
</template>
