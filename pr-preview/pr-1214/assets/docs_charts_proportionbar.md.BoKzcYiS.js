import{_ as ts}from"./chunks/PropsTable.CGAc7h40.js";import{_ as ps}from"./chunks/SlotsTable.CUNmAgfs.js";import{_ as cs}from"./chunks/EmitsTable.Bp11G1vJ.js";import"./chunks/theme.VPJp4lxG.js";import{t as is,B as rs}from"./chunks/axisChartCommon.DcUM-Srx.js";import{b as V,d as _s}from"./chunks/format.DrLjWwdn.js";import{t as os,p as hs}from"./chunks/hiddenSeries.BGnbo1Bl.js";import{p as ds,a as us,O as ys,d as fs,u as ms}from"./chunks/tokens.xQgu7ODm.js";import{_ as ws}from"./chunks/ChartContainer.vue_vue_type_script_setup_true_lang.vcyqpYBB.js";import{_ as gs}from"./chunks/ChartLegend.vue_vue_type_script_setup_true_lang.Gqah68sb.js";import{_ as vs}from"./chunks/ChartTooltip.vue_vue_type_script_setup_true_lang.CYshPEXp.js";import{J as P,aV as ks,b1 as bs,b9 as zs,ae as w,p as xs,A as H,bf as o,ao as x,a2 as $,M,E as d,o as s,a1 as R,a3 as W,r as g,F as T,an as Y,aG as O,Z as G,ak as E,l as m,ah as js,aw as S,D as a,ap as U,B as C}from"./chunks/framework.C2OQK7pm.js";const Cs=6,Ss=2,Ts="categorical",Ps=1.5;function Os(p,{tokens:r,hiddenSegments:l=[]}){const t=Rs(p),u=ds(p.palette,r,t.length,Ts),e=t.reduce((y,i)=>l.includes(i.name)?y:y+i.value,0),c=t.map(y=>l.includes(y.name)||!e?0:y.value/e*100),h=As(c);return t.map((y,i)=>({...y,color:u[i],hidden:l.includes(y.name),percent:c[i],width:h[i]}))}function As(p,r=Ps){const l=p.map(c=>c>0&&c<r?r:c),t=l.reduce((c,h)=>c+h,0),u=t-100;if(u<=0)return l;const e=l.reduce((c,h)=>c+Math.max(0,h-r),0);return e<=0?l.map(c=>c/t*100):l.map(c=>{const h=Math.max(0,c-r);return c-h*(u/e)})}function Rs(p){const l=(p.data??[]).map(i=>({row:i,value:is(i[p.valueColumn])})).filter(i=>i.value!==null&&i.value>=0),t=Math.max(Ss,p.maxSegments??Cs),u=l.length>t?t-1:l.length,e=Es(l,u),c=new Set,h=[],y=[];for(const i of l){if(e!==null&&i.value<e){y.push(i);continue}const b=Ds(i.row[p.categoryColumn]);h.push({name:Bs(b,c),label:b,value:i.value,rows:[i.row],isOthers:!1})}return y.length?[...h,{name:ys,label:us,value:y.reduce((i,b)=>i+b.value,0),rows:y.map(i=>i.row),isOthers:!0}]:h}function Es(p,r){return r>=p.length?null:p.map(t=>t.value).sort((t,u)=>u-t)[r-1]??null}function Ds(p){return p==null||p===""?"(Blank)":String(p)}function Bs(p,r){let l=p,t=2;for(;r.has(l);)l=`${p} (${t++})`;return r.add(l),l}const Ls=["data-size"],qs=["data-state","aria-label","onMouseenter","onFocus","onClick"],Is=3,D=P({__name:"ProportionBar",props:G({title:{},subtitle:{},dir:{},loading:{type:Boolean},error:{},data:{},category:{},value:{},maxSegments:{},size:{},hiddenSegments:{},format:{type:Function},palette:{}},{hiddenSegments:{default:()=>[]},hiddenSegmentsModifiers:{}}),emits:G(["select"],["update:hiddenSegments"]),setup(p,{emit:r}){const l=p,t=ks(p,"hiddenSegments"),u=r,e=E(),c=m(()=>l.dir??fs()),h={sm:{track:"h-2",hovered:"h-2.5",radius:3},md:{track:"h-3",hovered:"h-3.5",radius:4}},y=m(()=>l.size??"sm"),i=m(()=>h[y.value]),b=bs(),K=m(()=>!!(l.title||l.subtitle||b.actions)),X=m(()=>({data:l.data,categoryColumn:l.category,valueColumn:l.value,maxSegments:l.maxSegments,palette:l.palette,dir:c.value})),{tokens:J}=ms(e),j=m(()=>Os(X.value,{tokens:J.value,hiddenSegments:t.value})),A=m(()=>j.value.filter(n=>!n.hidden&&n.width>0)),Z=m(()=>!A.value.length),k=E(null),B=m(()=>A.value.some(n=>n.name===k.value)?k.value:null),Q=n=>B.value!==null&&B.value!==n.name,f=js({open:!1,x:0,y:0,items:[],rows:[]});function ss(n,v){L(n),q(v),f.open=!0}function as(n,v){L(n);const _=v.currentTarget.getBoundingClientRect();f.x=_.left+_.width/2,f.y=_.bottom,f.open=!0}function L(n){k.value=n.name,f.items=[{name:n.name,label:n.label,color:n.color,value:n.value,formattedValue:F(n.value),percent:n.percent,kind:"series"}],f.rows=n.rows}function q(n){f.open&&(f.x=n.clientX,f.y=n.clientY)}function I(){k.value=null,f.open=!1}function ns(n){u("select",{name:n.label,value:n.value,percent:n.percent,rows:n.rows})}function ls(n){return`${n.label}, ${F(n.value)}, ${V(n.percent)}`}function F(n){return l.format?l.format(n):_s(n)}const N=m(()=>j.value.map(n=>({name:n.name,label:n.label,color:n.color,hidden:n.hidden,hint:n.hidden?void 0:V(n.percent)})));function es(n){t.value=os(t.value,n,j.value.length)}return zs(()=>j.value.map(n=>n.name),n=>{t.value=hs(t.value,n)}),(n,v)=>(w(),xs(ws,{title:p.title,subtitle:p.subtitle,loading:p.loading,error:p.error,empty:Z.value,dir:c.value},H({default:o(()=>[s("div",{ref_key:"root",ref:e,class:R(["flex h-full w-full items-center",K.value&&"pt-2"])},[s("div",{"data-slot":"chart-track","data-size":y.value,class:R(["flex w-full items-center",i.value.track]),style:W({gap:`${Is}px`})},[(w(!0),g(T,null,Y(A.value,_=>(w(),g("button",{key:_.name,type:"button","data-slot":"chart-segment","data-state":k.value===_.name?"active":void 0,class:R(["transition-[height,width,opacity] duration-150 focus-visible:focus-ring motion-reduce:transition-none",k.value===_.name?i.value.hovered:i.value.track]),style:W({width:`${_.width}%`,backgroundColor:_.color,borderRadius:`${i.value.radius}px`,opacity:Q(_)?O(rs):void 0}),"aria-label":ls(_),onMouseenter:z=>ss(_,z),onMousemove:v[0]||(v[0]=z=>q(z)),onMouseleave:I,onFocus:z=>as(_,z),onBlur:I,onClick:z=>ns(_)},null,46,qs))),128))],14,Ls)],2),d(vs,{open:f.open,x:f.x,y:f.y,items:f.items,rows:f.rows,dir:c.value},H({_:2},[n.$slots.tooltip?{name:"default",fn:o(_=>[x(n.$slots,"tooltip",$(M(_)))]),key:"0"}:void 0]),1032,["open","x","y","items","rows","dir"])]),_:2},[n.$slots["title-suffix"]?{name:"title-suffix",fn:o(()=>[x(n.$slots,"title-suffix")]),key:"0"}:void 0,n.$slots.actions?{name:"actions",fn:o(()=>[x(n.$slots,"actions")]),key:"1"}:void 0,n.$slots.loading?{name:"loading",fn:o(()=>[x(n.$slots,"loading")]),key:"2"}:void 0,n.$slots.error?{name:"error",fn:o(_=>[x(n.$slots,"error",$(M(_)))]),key:"3"}:void 0,n.$slots.empty?{name:"empty",fn:o(()=>[x(n.$slots,"empty")]),key:"4"}:void 0,N.value.length>1?{name:"legend",fn:o(()=>[d(gs,{items:N.value,onChange:es,onHighlight:v[1]||(v[1]=_=>k.value=_)},null,8,["items"])]),key:"5"}:void 0]),1032,["title","subtitle","loading","error","empty","dir"]))}}),Fs={class:"flex h-full min-h-52 w-full items-center justify-center"},Ns={class:"flex w-full max-w-md flex-col gap-8"},Vs={class:"text-p-xs text-ink-gray-5"},Hs=P({__name:"ProportionSizes",setup(p){const r=[{source:"Organic",sessions:4820},{source:"Referral",sessions:2140},{source:"Paid",sessions:980}],l=["sm","md"];return(t,u)=>(w(),g("div",Fs,[s("div",Ns,[(w(),g(T,null,Y(l,e=>s("div",{key:e,class:"flex flex-col gap-2"},[s("span",Vs,'size="'+S(e)+'"',1),d(O(D),{data:r,category:"source",value:"sessions",size:e},null,8,["size"])])),64))])]))}}),$s={class:"flex h-full min-h-52 w-full items-center justify-center"},Ms={class:"flex w-full max-w-lg flex-col gap-3"},Ws={class:"text-p-sm text-ink-gray-5"},Gs=P({__name:"ProportionStorage",setup(p){const r=[{site:"shop.example.com",gb:214},{site:"crm.example.com",gb:96},{site:"docs.example.com",gb:41},{site:"staging.example.com",gb:12},{site:"blog.example.com",gb:7},{site:"status.example.com",gb:3},{site:"links.example.com",gb:1}],l=u=>`${u} GB`,t=E(null);return(u,e)=>(w(),g("div",$s,[s("div",Ms,[d(O(D),{data:r,category:"site",value:"gb","max-segments":4,format:l,title:"Disk used",subtitle:"374 GB of 500 GB",onSelect:e[0]||(e[0]=c=>t.value=c)}),s("p",Ws,[t.value?(w(),g(T,{key:0},[a(" Selected "+S(t.value.name)+" · "+S(Math.round(t.value.percent))+"% · "+S(t.value.rows.length)+" site(s) behind it ",1)],64)):(w(),g(T,{key:1},[a(" Select a segment — Others carries every site it groups. ")],64))])])]))}}),Us={class:"flex h-full min-h-52 w-full items-center justify-center"},Ys={class:"w-full max-w-md"},Ks=P({__name:"ProportionTickets",setup(p){const r=[{state:"Open",tickets:128},{state:"Waiting on customer",tickets:64},{state:"Resolved",tickets:412}],l=t=>`${t.toLocaleString("en-US")} tickets`;return(t,u)=>(w(),g("div",Us,[s("div",Ys,[d(O(D),{data:r,category:"state",value:"tickets",format:l,title:"Support queue",subtitle:"604 tickets this week"})])]))}}),ra=JSON.parse('{"title":"ProportionBar","description":"","frontmatter":{},"headers":[],"relativePath":"docs/charts/proportionbar.md","filePath":"docs/charts/proportionbar.md","lastUpdated":0}'),Xs={name:"docs/charts/proportionbar.md"},_a=Object.assign(Xs,{setup(p){const r=[{name:"title",description:"Heads the card. Left out, the chart draws no header row at all.",required:!1,type:"string"},{name:"subtitle",description:"A second line under the title, e.g. the period the numbers cover.",required:!1,type:"string"},{name:"dir",description:"Forces layout direction; defaults to document.documentElement.dir",required:!1,type:"ChartDir"},{name:"loading",description:"Draws the placeholder in place of the plot, for data still on its way.",required:!1,type:"boolean"},{name:"error",description:`Puts the chart in its error state and prints this message under it. Data
the chart cannot draw shows the empty state, not this one.`,required:!1,type:"string | null"},{name:"data",description:'The rows to draw. One row is one segment, before the "Others" grouping.',required:!0,type:"Record<string, any>[]"},{name:"category",description:"Row key holding the segment name.",required:!0,type:"string"},{name:"value",description:"Row key holding the segment size.",required:!0,type:"string"},{name:"maxSegments",description:'How many segments the bar holds, "Others" included. Past that it keeps the\nlargest `maxSegments - 1` and sums the tail into a single "Others" segment,\nnamed `OTHERS_KEY`. Defaults to 6 — the track runs out of readable width\nlong before the palette runs out of hues.',required:!1,type:"number"},{name:"size",description:"How thick the track is drawn: `'sm'` 8px, the default, for a strip under\nthe number it breaks down; `'md'` 12px where the breakdown is itself what\nthe card is about. The corner follows the thickness — the thinner track\ncannot carry the deeper one without rounding into capsules.",required:!1,type:'"sm" | "md"'},{name:"hiddenSegments",description:"Segments the legend has switched off, by name. Bind it with\n`v-model:hiddenSegments` to drive the legend from the app, or to keep what\na reader hid across a reload. Left unbound, the legend owns it.\n\n`hiddenSegments`, not the axis charts' `hiddenSeries`: a segment comes out\nof a `category` / `value` row rather than a column, so it is not a series\nunder another name. `spec/charts.md` records the ruling.",required:!1,type:"string[]",default:"[]"},{name:"format",description:"Prints every value the bar shows: the tooltip, and each segment's name.",required:!1,type:"ChartValueFormatter"},{name:"palette",description:"Defaults to `'categorical'`: the parts are unrelated, not steps of a ramp.",required:!1,type:"ChartPalette"}],l=[{name:"loading",description:"Replaces the whole placeholder, e.g. with a skeleton of the app's own.",type:"any"},{name:"error",description:"Replaces the message, e.g. to put a retry button beside it.",type:"{ error?: string | null | undefined; }"},{name:"empty",description:'Replaces the "no data" line, e.g. with a hint about the filters.',type:"any"},{name:"actions",description:"",type:"any"},{name:"title-suffix",description:"A mark right after the title, on the same line. The title truncates\naround it and it keeps its width. It renders in the title's font size, so\ncontent sized in `em` is smaller on a NumberCard than on a chart.",type:"any"},{name:"tooltip",description:'Replaces the tooltip body. `items` holds the hovered segment alone. A\nnamed segment carries one row, and "Others" every row it collapsed.',type:"ChartTooltipSlotProps"}],t=[{name:"select",description:`A segment was selected, by click or by Enter on the keyboard. The "Others"
segment carries every row it grouped, so a caller can drill into the tail
as well as into a named part.`,type:"[event: ProportionSegmentEvent]"},{name:"update:hiddenSegments",description:"Fired when the hidden segments changes.",type:"[value: string[]]"}];return(u,e)=>{const c=U("ComponentPreview"),h=U("ClientOnly");return w(),g("div",null,[e[4]||(e[4]=C('<h1 id="proportionbar" tabindex="-1">ProportionBar <a class="header-anchor" href="#proportionbar" aria-label="Permalink to “ProportionBar”">​</a></h1><p>One bar split into the parts that make up a total.</p><p>A single 100% stacked bar, also called a segmented bar. It is a ring unrolled, and it fits where a ring does not: a strip a few pixels tall under a number, splitting up the total above it. Use <code>DonutChart</code> when the breakdown fills the card, and this when the breakdown sits under something else. It is drawn as plain divs, with no echarts.</p><p>Three or four parts read at a glance. Past that the bar is still accurate, but the legend is doing the work, and <code>DonutChart</code> or <code>BarChart</code> gives each part a label of its own.</p><h2 id="parts-of-a-whole" tabindex="-1">Parts of a whole <a class="header-anchor" href="#parts-of-a-whole" aria-label="Permalink to “Parts of a whole”">​</a></h2><p><code>category</code> and <code>value</code> name the two columns the bar needs, the same pair <code>DonutChart</code> takes. One row is one segment, and each segment is as wide as its share of the total. <code>format</code> formats the numbers in the tooltip.</p><p>Rows draw in the order you pass them. They are not sorted by size, because a breakdown is usually written in the order it is read, and sorting would break that order.</p>',7)),d(h,null,{default:o(()=>[d(c,{name:"Charts-ProportionTickets","self-layout":""},{code:o(()=>[...e[0]||(e[0]=[s("div",{class:"language-vue"},[s("button",{title:"Copy Code",class:"copy"}),s("span",{class:"lang"},"vue"),s("pre",{class:"shiki shiki-themes tokyo-night github-light s_2cy0ei",tabindex:"0",dir:"ltr"},[s("code",null,[s("span",{class:"line"},[s("span",{class:"s_1zd9e2"},"<"),s("span",{class:"s_1uuh8p"},"script"),s("span",{class:"s_1i4ay4"}," setup"),s("span",{class:"s_1i4ay4"}," lang"),s("span",{class:"s_1jjt6x"},"="),s("span",{class:"s_w1p9wo"},'"'),s("span",{class:"s_2575z4"},"ts"),s("span",{class:"s_w1p9wo"},'"'),s("span",{class:"s_1zd9e2"},">")]),a(`
...
`),s("span",{class:"line"},[s("span",{class:"s_13ahmt"},"}")])])])],-1)])]),_:1}),d(ps,{data:l}),d(cs,{data:t})])}}});export{ra as __pageData,_a as default};
