...
| DAT-Q3 | H03 | decided | Keep both names and document a composable-to-error table with field differences. Follow-up (2026-09-16): maintainer renamed `FrappeRequestError` to `FrappeResourceError`; `FrappeResponseError` unchanged; both stay separate; the field table stands. | (none) |
...
- **L-shortcuts partly moved before RC.** Round 1 left the batch for 1.1. NAV-Q2 adds the `useKeyboardShortcut` setup guard before RC; the `showPlus`, dialog `title`/`paddingTop`, and array-overload docs stay in 1.1. Changed on 2026-09-24: the maintainer dropped the `showPlus` docs item and changed the prop's default before RC instead. It now defaults to `'auto'`, which draws `+` only between two text keys: "Ctrl+K", but "⌘K" and "⇧F2". The prop stays but is not promoted in docs.
...
| ED-Q8 | M-editor/A | decided | Export one owned narrow options type for both menu components | (none) <br>Follow-up (2026-09-16): maintainer confirmed the narrow type after a consumer census (Gameplan, Builder, Suite, frappe/ui, Wiki, CRM, Helpdesk, Books, Insights, LMS): only Wiki passes Floating UI function forms of `flip`, `shift`, `hide` and `placement`, and Suite passes `getReferencedVirtualElement`, which TipTap never read from the bag. The runtime still spreads unknown keys through; the type stays narrow <br>Follow-up (2026-09-16): Barista nit; `offset` is `number` only, `0` disables the gap, the same as `Popover`. TipTap guards the offset middleware with `if (this.floatingUIOptions.offset)`, so `0` and `false` behave alike |
...
- **Row state attributes (LIST-Q5).** In `ListRowBase.vue:2-9`: `data-state="active|inactive"`, `data-selected`, `data-interactive`. The 2 Builder candidates are false positives on app-authored divs; actual app selector migrations are 0. There are 9 own-tree sites. PR title needs `!`.
...
- **Sort slot (LIST-Q8).** `#suffix` → `#sort-indicator` on `ListHeaderCellSort` (`ListHeaderCellSort.vue:22-51`, `types.ts:101-115`); placement stays. Measured app impact is 5 live sites across 2 files: Gameplan 2 and Suite Drive 3. Gameplan also has 2 documentation examples. PR title needs `!`.
...
| QID | Covers | Status | Choice | Answer |
|---|---|---|---|---|
| PKG-Q1 | H33, H34 | decided | Add a Tailwind v3 peer range and add `@floating-ui/vue` as a direct dependency | (none) <br>Note: PKG-Q3 needs Tailwind 3.4, so the peer range is `>=3.4.0 <4`, not `>=3.2.7 <4` <br>Follow-up (2026-09-16): Greptile flagged the `VIA_PEER` exemption in `package.test.ts`, which let shipped code import packages frappe-ui does not declare. `shiki`, `@shikijs/transformers` and `@vue/compiler-dom` are now optional peers and the exemption is gone. `markdown-it` stays undeclared, because the import is type-only and the build erases it |
