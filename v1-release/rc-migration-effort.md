...
| M-packaging/A lucideIcons | `lucideIcons` default `true` -> `false` | T1 | 1 | 0 | 82 | yes | **trap** | **break**, trap clause |
...
| M-packaging/A lucideIcons | The Vite plugin stops installing the `~icons` resolver and the two unplugins. Measured 2026-09-15 with `packaging-v1 --dry-run`: GP `frontend/vite.config.ts` is the one config that needs the option; Builder and Suite already pass `lucideIcons: true`; apps/frappe does not use the plugin. The own-tree number is the `~icons/lucide/*` imports converted to class icons, in 12 shipped files. | `frappeui\(` without `lucideIcons` in the same options object | 1 | 0 | - | - | 0 | - | - | 0 | 0 | 82 |
