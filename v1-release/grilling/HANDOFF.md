...
All nine pages are folded in; every question is decided or deferred. `v1-release/rc-work-list.md` is the plan: 18 PRs in a fixed order, each listing the QIDs it implements, files, consumer sites, migration-guide lines, dependencies, and size. Two conflicts in its "Conflicts and gaps" list need the maintainer's call before PR 7 and PR 9 (state vocabulary `selected` vs `active`; Tooltip open controls). The other gaps are spec/ADR edits that belong inside the PR they relate to.
