...
function controlledWindow(options = {}) {
    return defineComponent({
        setup() {
            const mode = ref(options.initialMode ?? 'docked');
            return { mode };
        },
        render() {
            return [
                ...
                h(FloatingWindow, {
                    ...
                    mode: this.mode,
                    'onUpdate:mode': (value) => (this.mode = value),
...
function dragResize(name, from, delta) {
...
describe('FloatingWindow', () => {
    ...
    it('lets the host drive the window through v-model:mode', () => {
        const Host = defineComponent({
            ...
            render() {
                return [
                    h('button', { 'data-cy': 'go-float', onClick: () => (this.mode = 'floating') }, 'float'),
                    h(FloatingWindow, {
                        title: 'Messages',
                        mode: this.mode,
                        'onUpdate:mode': (value) => (this.mode = value),
    ...
    it('pins the previously detached window when a second one pops out', () => {
        ...
        cy.contains('.floating-window', 'Window A')
            ...
            .click();
        cy.contains('.floating-window', 'Window A').should('have.attr', 'data-state', 'floating');
        cy.contains('.floating-window', 'Window B')
            .find('[aria-label="Pop out"]')
            .click();
        // B is now floating; A was pinned back into the docked layout.
        cy.contains('.floating-window', 'Window B').should('have.attr', 'data-state', 'floating');
        cy.contains('.floating-window', 'Window A')
