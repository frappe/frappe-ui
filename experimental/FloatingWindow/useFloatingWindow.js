...
let activeDocker = null;
function setActiveDocker(docker) {
    activeDocker = docker;
 ...
 */
export function useFloatingWindow(panel, handle, options = {}) {
    ...
    const mode = ref(saved.value.mode);
    ...
    function startDrag(event) {
        ...
        const stopMove = useEventListener('pointermove', (e) => {
     ...
     */
    function startResize(event, dir = { x: 1, y: 1 }) {
    ...
    watch(() => mode.value !== 'docked', (detached) => {
        if (detached) {
            if (activeDocker && activeDocker !== dock)
                activeDocker();
