    ...
    <DesktopShell>
      ...
      <div class="px-3 pb-10 pt-4 sm:px-5">
        ...
        >
          ...
          <!-- Group headers are plain full-width blocks between rows: the List
               container isn't a grid, and breaking row-to-row adjacency also
               drops the divider above each group's first row. Folders and files
               interleave within a bucket, ordered by the active column sort. -->
          <template v-for="group in groups" :key="group.key">
            <p class="px-3 pb-1 pt-4 text-sm font-medium text-ink-gray-5">
              {{ group.label }}
            </p>
            <ListRow
