...
const theme = ref<'gray' | 'blue' | 'green' | 'red'>('gray')
const variant = ref<'solid' | 'subtle' | 'outline' | 'ghost'>('subtle')
const size = ref<'sm' | 'md' | 'lg' | 'xl' | '2xl'>('sm')
...
const code = computed(() => {
  ...
  if (icon.value) attrs.push(`icon-left="lucide-plus"`)
    ...
    >
      ...
      >
        <Button
          ...
          :variant="variant"
          :size="size"
          :icon-left="icon ? 'lucide-plus' : undefined"
          :loading="loading"
          :disabled="disabled"
        >
          {{ label }}
