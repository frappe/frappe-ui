...
const EMPTY_SLOT_TYPES = new Set(['any', 'void[]', 'void', '{}'])

function isEmptySlotType(type: string | undefined): boolean {
  return !type || EMPTY_SLOT_TYPES.has(type)
}

function displayType(type: string | undefined): string {
  if (isEmptySlotType(type)) return '—'
  return formatTypeStr(type!)
}

function isLongRow(x: ItemProp): boolean {
  const typeLong =
    !x.deprecated && !isEmptySlotType(x.type) && isLongType(x.type)
  const descLong =
