...
<template>
  <div class="grid gap-3">
    ...
    <template v-if="activeTabLabel === 'Font Size'">
      ...
      >
        ...
        <div class="mt-2 text-xs opacity-60">
          {{ item.value[0] }}
          · lh {{ item.value[1]?.lineHeight }} · ls
          {{ item.value[1]?.letterSpacing }} · fw
    ...
    <template v-if="activeTabLabel === 'Line Height'">
      ...
      >
        <div class="text-xs opacity-60 mb-1">leading-{{ item.name }}</div>

        <p class="text-base my-2" :style="{ lineHeight: item.value }">
          {{ para }}
        </p>
