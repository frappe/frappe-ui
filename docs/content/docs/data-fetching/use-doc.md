...
- `setValue` — a [`useCall`](./use-call.md)-shaped member;
  ...
  the fields the loaded doc has; the response replaces them when it lands. It
  **rejects** when the update fails. `doc` reverts unless a response or fetch
  has replaced the doc since; each list row field reverts unless the row no
  longer holds the optimistic value, e.g. after a list reload.
