...
Every sub-plugin except `frappeTypes` is **enabled by default**. Pass `false` to
disable one, or an options object to override its defaults. `frontendRoute` and
`frappeTypes` need explicit configuration — `frontendRoute` sets the app route,
and `frappeTypes` needs an `input` map of app names to doctype names.
...
- **Dev server site banner** — prints clickable URLs for every site the app is
  installed on, on startup.
...
so importing one component from it makes the browser request the barrel _and_
every module it re-exports: echarts, TipTap, CodeMirror, socket.io included.
Vite's dependency optimizer then discovers those packages mid-crawl and triggers
a full "optimized dependencies changed, reloading" round trip.

The `barrelImports` sub-plugin rewrites `import { Button } from 'frappe-ui'` at
transform time into a deep import of the module that actually declares `Button`,
so authors keep writing the barrel import while dev only ever loads what's used.
It's on by default — no configuration needed for the common case.
...
side-effect imports are left alone, as is any named import the plugin can't map
— it falls back to a residual barrel import so behavior never silently changes.
...
imports, and the ten are optional peer dependencies. Both of Vite's bundlers
resolve those imports ahead of time, so a package the app did not install stops
...
and stops the dev server, which exits during dependency pre-bundling:
...
`loadLanguage` reaches it, through a Rollup hook for the build and an esbuild
twin for pre-bundling. Both then succeed, and the error names the package to
install:
...
`frappe-ui/code-editor` in your app. An app file of its own at
`src/code-editor/languages.ts` is left alone, and so is a language package the
app imports itself.
...
Turning it off means installing all ten packages. See
...
`~icons/lucide/*` or writes `<LucideName />` tags. See the [Icons](./icons) page
for the three forms.
...
are regenerated only when the source DocType changes. Off by default, and unlike
the other sub-plugins it doesn't accept `true` — pass an options object with
`input` to turn it on, since it needs that map to know which doctypes to
generate.
