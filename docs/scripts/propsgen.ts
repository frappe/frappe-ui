...
// run — unlike the API rows, which the TypeScript checker orders. `null` means
...
// run's output, so a partial run is for iterating, not for committing. CI
// compares the rows themselves and ignores their order (`--check`), so a
// committed partial run does not fail the build — it just reads differently.
